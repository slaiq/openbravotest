...
function checkdatevalidation(startValue,endValue) 
{   
	
	if((startValue.length=='0')||endValue.length=='0')
		{
		OBAlert("Please Enter Start date and End date");
		return false;
		}
	...
	if (fromdate.getTime() > todate.getTime())
	{
		...
		OBAlert("End date Should Be greater Than The Start Date");
		 return 1;
	} 
	else
		{
...
function selectBoxAjaxPaging(options) {
	...
	if ("small" == options.size) {
		...
		smallCss += 'height: 30px !important;'
		smallCss += '}'
		...
		smallCss += 'line-height: 25px !important;'
		smallCss += 'height: 28px !important;'
		smallCss += '}'
