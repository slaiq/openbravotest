...
if (isc.ListGrid == null) {
	...
	isc.DateGrid
			.addProperties({
				...
				initWidget : function() {
					this.shortDayNames = getShortHijriDayNames();
					this.shortDayTitles = getShortHijriDayNames();
				...
				getHijCellDate : function( record, rowNum, colNum) {
					...
					return calendar.newDate(date.getFullYear(), parseInt(date.getMonth())+1, date.getDate());
				},
				...
				getCellStyle : function(record, rowNum, colNum) {
					...
					var  isDisabled = this.dateIsDisabled(date), 
					isOtherMonth = currentMonth != this.workingMonth, style = this.Super("getCellStyle", arguments);
					;
				...
				setStartDate : function(startDate) {
					...
					if(startDate.year() >= 1250 && startDate.year() <=1500 ){
						...
						var hijriDate =  calendar.newDate().fromJSDate(startDate);
						this.workingMonth = hijriDate.month();
						this.workingMonth = month;
						this.startDate = weekStart;
						this.buildCalendarData();
						this.markForRedraw();
			    ...
			    getPreviousMonthDate : function(currentDate, days) {
			    	...
			    	var daysInPreviousMonth = calendar.daysInMonth(year, month-1);
			    	var newDate = calendar.newDate(year, month-1, daysInPreviousMonth);
			    	 
			    	if (days > 0)
			    		newDate = calendar.newDate(year, month-1, daysInPreviousMonth-(days));
			    	console.log("Current Month startdate: "+currentDate+" Days: "+days);
				...
				buildCalendarData : function(startDate) {
					...
					var sType = this.getDateType(startDate);
					if(sType == "Object"){
						var records = [], date = startDate, startMonth = startDate.month(),
						yearWrap = (startMonth == 12 || this.workingMonth == 12);
						
						var calendar = $.calendars.instance("ummalqura");
						
						sDate2 =this.getNextMonthStart(this.workingMonth, startDate);
						var jsDate2 = sDate2.toJSDate(),
						...
						for (var i = 0; i <= counter; i++) {
							...
							if (i == counter && thisDate.month() != this.workingMonth) {
								...
								var thisWeekRecord = this.getWeekRecord(thisDate);
								records.add(thisWeekRecord);//// thisWeekRecord.rowStartDate.toJSDate()
				...
				getNextMonthStart : function(workingmonth, startDate) {
					...
					var nextMonthStart = calendar.newDate(year, workingmonth, 1);*/
				...
				getWeekRecord : function(date) {
					...
					if(sType == "Object"){
						...
						for (var i = 0; i < 7; i++) {
							...
							//if (this.showWeekends || !weekendDays.contains(hijriDate.dayOfWeek())) {
