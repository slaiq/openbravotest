...
<%@page import="sa.elm.ob.hcm.ad_reports.employeeretirement.EmployeeRetirementVO"%>
...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    List<EmployeeRetirementVO> typeList =  (List<EmployeeRetirementVO> )request.getAttribute("inpEmployeeTypeList");
    List<EmployeeRetirementVO> gradefromList =  (List<EmployeeRetirementVO> )request.getAttribute("inpEmployeeGradeFromList");
    List<EmployeeRetirementVO> gradetoList =  (List<EmployeeRetirementVO> )request.getAttribute("inpEmployeeGradeToList");
    List<EmployeeRetirementVO> linemanagerList =  (List<EmployeeRetirementVO> )request.getAttribute("inpLineManagerList");
                              ...
                              <div>
                                 <span class="dojoTabcurrentfirst">
                                    <div>
                                       ...
                                       <%= Resource.getProperty("hcm.empret.title",lang).replace("'", "\\\'") %>
                                       </a></span>
                                    </div>
                                 </span>
                              </div>
                                                 ...
                                                 <option value="0"><%= Resource.getProperty("hcm.--select--",lang).replace("'", "\\\'") %></option>
                                                  <% 
                                                      if (typeList!=null && typeList.size() > 0) 
                                                          for(EmployeeRetirementVO sup : typeList) { %>    
                                                 ...
                                                 <option value="0"><%= Resource.getProperty("hcm.--select--",lang).replace("'", "\\\'") %></option>
                                                  <% 
                                                      if (gradefromList!=null && gradefromList.size() > 0) 
                                                          for(EmployeeRetirementVO sup : gradefromList) { %>    
                                                          <option value="<%= sup.getPortId() %>"><%= sup.getPortName() %></option>
                                                      <% } %>   
                                                </select>
                                             </td>
                                                 ...
                                                 <option value="0"><%= Resource.getProperty("hcm.--select--",lang).replace("'", "\\\'") %></option>
                                                  <% 
                                                      if (gradetoList!=null && gradetoList.size() > 0) 
                                                          for(EmployeeRetirementVO sup : gradetoList) { %>    
                                                          <option value="<%= sup.getPortId() %>"><%= sup.getPortName() %></option>
                                                      <% } %>   
                                                </select>
                                             </td>
                                                 ...
                                                 <option value="0"><%= Resource.getProperty("hcm.--select--",lang).replace("'", "\\\'") %></option>
                                                  <% 
                                                      if (linemanagerList!=null && linemanagerList.size() > 0) 
                                                          for(EmployeeRetirementVO sup : linemanagerList) { %>    
                                                          <option value="<%= sup.getPortId() %>"><%= sup.getPortName() %></option>
                                                      <% } %>   
                                                </select>
                                             </td>
...
function reloadWindow() {
    submitCommandForm('DEFAULT', false, null, 'EmployeeRetirement', '_self', null, false);
    return false;
}
...
function generateReport() {
   ...
   // alert("gradefrom:"+parseInt($("#inpEmployeeGradeFrom").val())<parseInt($("#inpEmployeeGradeTo").val()));
      if($("#inpEmployeeGradeFrom").val()!=0 && $("#inpEmployeeGradeTo").val()!=0){
    if($("#inpEmployeeGradeFrom").val()>$("#inpEmployeeGradeTo").val()) {
        OBAlert("FromGrade should be less than or equal to ToGrade");
        return false;
    }  }
    ...
    if(valid==1){
        openServletNewWindow('PRINT_PDF', true, null, 'EmployeeRetirement', null, false, '700', '1000', true);
        return false;
    }
}
...
//inpEndDateH
//document.getElementById("inpEffectiveDate").style.backgroundColor="#F5F7F1";  
...
function getDepartmentList(empId){  
  ...
  //  document.getElementById("inpDepartment").value = "00";
          ...
          $("#inpDepartment").select2(selectBoxAjaxPaging({
            url : function() {
                return '<%=request.getContextPath()%>/sa.elm.ob.hcm.ad_reports.employeeretirement/EmployeeRetirement?inpAction=getDepartmentList&inpEmpId='+empId
            },
            size : "small"
        }
          ));
  ...
  <%--       $.post("<%=request.getContextPath()%>/sa.elm.ob.hcm.ad_reports.employeeretirement/EmployeeRetirement", {
