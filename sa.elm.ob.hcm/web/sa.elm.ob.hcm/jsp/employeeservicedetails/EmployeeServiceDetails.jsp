...
<HEAD>
<TITLE><%= Resource.getProperty("hcm.empservicedts.employeeservicedetails",lang) %></TITLE>
...
function onResizeDo() {
   ...
   <BODY onresize="onResizeDo();" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" id="paramBody">
      <FORM id="form" method="post" action="" name="frmMain">
         ...
         <INPUT type="hidden" name="inpEffectDate" id="inpEffectDate" value=""></INPUT>          
                              ...
                              <div>
                                 <span class="dojoTabcurrentfirst">
                                    <div>
                                       ...
                                       <%= Resource.getProperty("hcm.empservicedts.employeeservicedetails",lang).replace("'", "\\\'") %>
                                       </a></span>
                                    </div>
                                 </span>
                              </div>
                                    ...
                                    <TD>
                                       ...
                                       <TABLE  align="center" width="100%">
                                          ...
                                          <tr>
                                           ...
                                           <tr>
                                             <td class="TitleCell"><span class="LabelText" id="inpEffectiveDateLabel"> <b> <%=Resource.getProperty("hcm.empservicedts.effectivedate", lang).replace("'", "\\\'")%>
                                                </b>
                                                </span>
                                             </td>                                             
                                             <td class="TextBox_ContentCell">
                                             <% if(request.getAttribute("inpEffectiveDate")!= null ) {%>
	                                                <input type="text" id="inpEffectiveDate" class="dojoValidateValid TextBox_btn_OneCell_width required" onchange="document.getElementById('messageBoxID').style.display = 'none';onChangeField();"  value="<%= request.getAttribute("inpEffectiveDate") %>" maxlength="10" name="inpEffectiveDate" ></input> 
	                                            <%}
	                                             else{ %>
...
function reloadWindow() {
    submitCommandForm('DEFAULT', false, null, 'EmployeeServiceDetails', '_self', null, false);
    return false;
}
...
function generateReport() {
	document.getElementById("inpEffectDate").value = document.getElementById("inpEffectiveDate").value;
    ...
    if (document.getElementById("inpEmpNo").value == 0) {
        OBAlert('<%=Resource.getProperty("hcm.empservicedts.empnocannotbeempty",lang).replace("'","\\\'") %>');
        return false;
    }
    if (document.getElementById("inpEffectiveDate").value == "") {
        OBAlert('<%=Resource.getProperty("hcm.empservicedts.selecteffectivedate",lang).replace("'","\\\'") %>');
        return false;
    }
    //Checking Date format validation
    var inpEffDate=$("#inpEffectiveDate").val();
    var dateformat = /^(0?[1-9]|[12][0-9]|3[01])[\/\-](0?[1-9]|1[012])[\/\-]\d{4}$/;
    if(inpEffDate!="")
    {
        if(inpEffDate.match(dateformat))
            {
    ...
    openServletNewWindow('PRINT_PDF', true, null, 'EmployeeServiceDetails', null, false, '700', '1000', true);
    return false;
}
