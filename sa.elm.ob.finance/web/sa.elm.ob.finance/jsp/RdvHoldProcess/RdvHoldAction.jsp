...
<%@page import="sa.elm.ob.finance.ad_process.RDVProcess.RdvHoldProcess.RdvHoldActionVO"%>
...
<%@page import="java.util.List,java.util.ArrayList"%>
 ...
 String strholdType =  "0:" + Resource.getProperty("finance.Select", lang) + ";";
...
<BODY onresize="onResizeDo();" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" id="paramBody" onunload="refreshParent();">
    <FORM id="form" method="post" action="" name="frmMain">
        ...
        <INPUT type="hidden" name="inpRDVTxnId" id="inpRDVTxnId" value="<%= request.getAttribute("inpRDVTxnId") %>"></INPUT>
        <INPUT type="hidden" name="isTxn" id="isTxn" value="<%= request.getAttribute("isTxn") %>"></INPUT>
        ...
        <INPUT type="hidden" name="inplineno" id="inplineno" value="<%= request.getAttribute("inplineno") %>"></INPUT>
         ...
         <INPUT type="hidden" name="inpholdtype" id="inpholdtype" value="<%= request.getAttribute("inpholdtype") %>"></INPUT> 
          <INPUT type="hidden" name="inpholdThershold" id="inpholdThershold" value=""></INPUT>   
         ...
         <input type="hidden" name = "inpDefaultHoldAcctType" id="inpDefaultHoldAcctType" value="<%= request.getAttribute("inpHoldAccountType") %>"></input>   
    ...
    <TD>
      <TABLE cellspacing="0" cellpadding="0" class="Popup_ContentPane_NavBar" id="tdToolBar">
        <TR class="Popup_NavBar_bg"><TD></TD>
          ...
          <TD class="Popup_NavBar_Popup_title_cell"><SPAN><%=Resource.getProperty("finance.hold.action",lang)%></SPAN></TD>
          <TD class="Popup_NavBar_separator_cell"></TD>
        </TR>
      </TABLE>
    </TD>
     ...
     <table cellspacing="0" cellpadding="0" class="Popup_ContentPane_InfoBar">
        <tbody><tr>
          ...
          <td class="Popup_InfoBar_text_table">
            <table>
              <tbody><tr>
                <td class="Popup_InfoBar_text" id="processHelp"><%=Resource.getProperty("finance.hold.action",lang)%> </td>
              </tr>
            </tbody></table>
          </td>
        </tr>
                                                          ...
                                                          <TR>
                                                            <td colspan="3" rowspan="5">
                                                                ...
                                                                <div id="jqgrid" style="width:100%; ">
                                                                   <div align="center"><table id="HoldList" class="scroll" cellpadding="0" cellspacing="0" width="100%"></table><div id="pager" class="scroll" style="text-align: center;"></div></div>
                                                                </div>
                                                            </td>
                                                            </TR>
...
<%--  <% List<RdvHoldActionVO> inpbank = (ArrayList<RdvHoldActionVO>)  request.getAttribute("inpbpartnername");
...
<% List<RdvHoldActionVO> inpholdType = (ArrayList<RdvHoldActionVO>)  request.getAttribute("inpholdtype");

//strbanklist = "{";
for (RdvHoldActionVO vo : inpholdType){
    if(thershold==null)
    thershold = vo.getThershold();
strholdType += vo.getHoldId() + ":" +  vo.getHoldname() + ";";
}
//strbanklist += "}";
%>

<% List<RdvHoldActionVO> inpinvoice = (ArrayList<RdvHoldActionVO>)  request.getAttribute("inpinvoiceNo");
//strbanklist = "{";
for (RdvHoldActionVO vo : inpinvoice){
...
document.getElementById("inpinvoiceNo").value="<%=strinvoicelist.substring(0,strinvoicelist.lastIndexOf(";"))%>";
 ...
 <% List<RdvHoldActionVO> inpuniquecode = (ArrayList<RdvHoldActionVO>)  request.getAttribute("inpuniquecode"); 
//strbanklist = "{";
...
document.getElementById("inpholdtype").value="<%=strholdType.substring(0,strholdType.lastIndexOf(";"))%>";
document.getElementById("inpholdThershold").value="<%=thershold%>";
var Removenegative='<%= Resource.getProperty("finance.hold.action.remove.negative", lang) %>';
var addpositive='<%= Resource.getProperty("finance.hold.action.add.positive", lang) %>';
var askSave='<%= Resource.getProperty("finance.hold.action.askSave", lang) %>';
var askDelete='<%= Resource.getProperty("finance.hold.action.askDelete", lang) %>';
var bpmandatory='<%= Resource.getProperty("finance.hold.action.associatedbp.mandatory", lang) %>';
var holdamtshouldnotnegrdv='<%= Resource.getProperty("finance.hold.action.holdamount.shouldnot.negative", lang) %>';
var holdamtgreterthanmatchamt='<%= Resource.getProperty("finance.hold.action.holdamt.greater.than.matchamt", lang) %>';
var holdamtshouldnotneg='<%= Resource.getProperty("finance.hold.type.should.mandatory", lang) %>';
var select ='<%= Resource.getProperty("finance.Select", lang) %>';
var valueschanged ='<%= Resource.getProperty("finance.hold.valueschanged", lang) %>';
var versionstatusinvoice='<%= Resource.getProperty("finance.hold.version.status.invoiced", lang) %>';
var holdamtnotblank  = '<%= Resource.getProperty("finance.hold.holdamount.not.blank", lang) %>';
var uniquecodemandatory='<%= Resource.getProperty("finance.hold.action.uniquecode.mandatory", lang) %>';
var select='<%= Resource.getProperty("finance.Select", lang) %>';
var sequence='<%= Resource.getProperty("finance.hold.action.sequence", lang) %>';
var action='<%= Resource.getProperty("finance.hold.action.action", lang) %>';
var txnAppNo='<%= Resource.getProperty("finance.hold.action.txn.application.no", lang) %>';
var action='<%= Resource.getProperty("finance.hold.action.action", lang) %>';
var actionDate='<%= Resource.getProperty("finance.hold.action.action.date", lang) %>';
var matchAmt='<%= Resource.getProperty("finance.hold.action.match.amt", lang) %>';
var holdTypes='<%= Resource.getProperty("finance.hold.action.hold.type", lang) %>';
var holdPercent='<%= Resource.getProperty("finance.hold.action.hold.percentage", lang) %>';
var holdAmount='<%= Resource.getProperty("finance.hold.action.hold.amount", lang) %>';
var actionReason='<%= Resource.getProperty("finance.hold.action.action.reason", lang) %>';
var actionJustification='<%= Resource.getProperty("finance.hold.action.action.justification", lang) %>';
var asociatedBp='<%= Resource.getProperty("finance.hold.action.associated.bp", lang) %>';
var bpName='<%= Resource.getProperty("finance.hold.action.bp.name", lang) %>';
var freezeHold='<%= Resource.getProperty("finance.hold.action.freeze.hold", lang) %>';
var amrasafNo='<%= Resource.getProperty("finance.hold.action.amrasaf.no", lang) %>';
var amrasafAmt='<%= Resource.getProperty("finance.hold.action.amrasaf.amount", lang) %>';
var deductionAcctType='<%= Resource.getProperty("finance.hold.action.hold.deduction.acct.type", lang) %>';
var uniquecode='<%= Resource.getProperty("finance.hold.action.hold.uniquecode", lang) %>';
var uniquecodeName='<%= Resource.getProperty("finance.hold.action.hold.uniquecodename", lang) %>';
var holdAmtNotZero='<%= Resource.getProperty("finance.hold.amount.should.not.zero", lang) %>';
var add='<%= Resource.getProperty("finance.hold.add", lang) %>';
var remove='<%= Resource.getProperty("finance.hold.remove", lang) %>';
var edit='<%= Resource.getProperty("finance.hold.edit", lang) %>';
var save='<%= Resource.getProperty("finance.hold.save", lang) %>';
var cancel='<%= Resource.getProperty("finance.hold.cancel", lang) %>';
var reload='<%= Resource.getProperty("finance.hold.reload", lang) %>';
var deleteicon='<%= Resource.getProperty("finance.hold.delete", lang) %>';
var expense='<%= Resource.getProperty("finance.hold.expense", lang) %>';
var adjustment='<%= Resource.getProperty("finance.hold.adjustment", lang) %>';
var holdAmtGreater='<%= Resource.getProperty("finance.hold.holdamtgreater", lang) %>';
var holdTypeMore='<%= Resource.getProperty("finance.hold.holdtypemore", lang) %>';
...
<script type="text/javascript" src="../web/sa.elm.ob.finance/js/RdvHoldProcess/RdvHoldAction.js"></script>
