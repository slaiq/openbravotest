 ...
 *************************************************************************
 * All Rights Reserved.
 * Contributor(s): Qualian Technologies
 ************************************************************************
 */
OB.EFIN = {};
OB.EFIN.AddPayment = {};
OB.EFIN.AddPayment = {
  ...
  ordInvTransformData: function (newData, dsResponse) {
    ...
    for (i = 0; i < data.length; i++) {
      record = data[i];
      ids = OB.EFIN.AddPayment.orderAndRemoveDuplicates(record.id);
      record.id = ids;
      record.invoiceNo = OB.EFIN.AddPayment.orderAndRemoveDuplicates(record.invoiceNo);
      record.salesOrderNo = OB.EFIN.AddPayment.orderAndRemoveDuplicates(record.salesOrderNo);
...
OB.EFIN.AddPayment.onLoad = function (view) {
  ...
  OB.EFIN.AddPayment.paymentMethodMulticurrency(view, view.theForm, !payment);
  OB.EFIN.AddPayment.reloadLabels(form);
  glitemGrid.fetchData();
  creditUseGrid.fetchData();
  orderInvoiceGrid.selectionChanged = OB.EFIN.AddPayment.selectionChanged;
  orderInvoiceGrid.userSelectAllRecords = OB.EFIN.AddPayment.userSelectAllRecords;
  orderInvoiceGrid.deselectAllRecords = OB.EFIN.AddPayment.deselectAllRecords;
  orderInvoiceGrid.dataProperties.transformData = OB.EFIN.AddPayment.ordInvTransformData;
  glitemGrid.removeRecordClick = OB.EFIN.AddPayment.removeRecordClick;
  creditUseGrid.selectionChanged = OB.EFIN.AddPayment.selectionChangedCredit;
  orderInvoiceGrid.dataArrived = OB.EFIN.AddPayment.ordInvDataArrived;

  form.isCreditAllowed = form.getItem('received_from').getValue() !== undefined && form.getItem('received_from').getValue() !== null;
  OB.EFIN.AddPayment.checkSingleActionAvailable(form);
...
OB.EFIN.AddPayment.addNewGLItem = function (grid) {
...
OB.EFIN.AddPayment.paymentMethodMulticurrency = function (view, form, recalcConvRate) {
  ...
  callback = function (response, data, request) {
    ...
    if (data.isWrongFinancialAccount && trxtype === "") {
      ...
      if (recalcConvRate) {
        ...
        OB.EFIN.AddPayment.updateConvertedAmount(view, form, false);
      }
    }
...
OB.EFIN.AddPayment.checkSingleActionAvailable = function (form) {
...
OB.EFIN.AddPayment.financialAccountOnChange = function (item, view, form, grid) {
  var affectedParams = [];
  OB.EFIN.AddPayment.paymentMethodMulticurrency(view, form, true);
  OB.EFIN.AddPayment.checkSingleActionAvailable(form);
  affectedParams.push(form.getField('c_currency_id_readonly_logic').paramId);
  OB.EFIN.AddPayment.recalcDisplayLogicOrReadOnlyLogic(form, view, affectedParams);
};

OB.EFIN.AddPayment.paymentMethodOnChange = function (item, view, form, grid) {
...
OB.EFIN.AddPayment.currencyOnChange = function (item, view, form, grid) {
...
OB.EFIN.AddPayment.transactionTypeOnChangeFunction = function (item, view, form, grid) {
  var ordinvgrid = form.getItem('order_invoice').canvas.viewGrid,
...
OB.EFIN.AddPayment.actualPaymentOnChange = function (item, view, form, grid) {
  var issotrx = form.getItem('issotrx').getValue();
  if (issotrx) {
    OB.EFIN.AddPayment.distributeAmount(view, form, true);
    OB.EFIN.AddPayment.updateConvertedAmount(view, form, false);
  }
};

OB.EFIN.AddPayment.orderInvoiceOnLoadGrid = function (grid) {
  ...
  if ((issotrx || !payment) && (grid.selectedIds.length === 0)) {
    OB.EFIN.AddPayment.distributeAmount(this.view, this.view.theForm, false);
  } else {
    OB.EFIN.AddPayment.updateInvOrderTotal(this.view.theForm, grid);
  }
  OB.EFIN.AddPayment.tryToUpdateActualExpected(this.view.theForm);
};

OB.EFIN.AddPayment.glitemsOnLoadGrid = function (grid) {
  if (!grid.isReady) {
  ...
  grid.isReady = true;
  OB.EFIN.AddPayment.updateGLItemsTotal(this.view.theForm, 0, false);
  OB.EFIN.AddPayment.tryToUpdateActualExpected(this.view.theForm);
};

OB.EFIN.AddPayment.creditOnLoadGrid = function (grid) {
  grid.isReady = true;
  OB.EFIN.AddPayment.updateCreditTotal(this.view.theForm);
  OB.EFIN.AddPayment.tryToUpdateActualExpected(this.view.theForm);
};

OB.EFIN.AddPayment.tryToUpdateActualExpected = function (form) {
  ...
  if (orderInvoiceGrid.isReady && glitemGrid.isReady && creditGrid.isReady) {
    OB.EFIN.AddPayment.updateActualExpected(form);
  }
};

OB.EFIN.AddPayment.orderInvoiceAmountOnChange = function (item, view, form, grid) {
  OB.EFIN.AddPayment.updateActualExpected(form);
  OB.EFIN.AddPayment.updateInvOrderTotal(form, grid);
};

OB.EFIN.AddPayment.orderInvoiceTotalAmountOnChange = function (item, view, form, grid) {
  OB.EFIN.AddPayment.updateActualExpected(form);
  OB.EFIN.AddPayment.updateTotal(form);
};

OB.EFIN.AddPayment.glItemTotalAmountOnChange = function (item, view, form, grid) {
  OB.EFIN.AddPayment.updateActualExpected(form);
  OB.EFIN.AddPayment.updateTotal(form);
};

OB.EFIN.AddPayment.distributeAmount = function (view, form, onActualPaymentChange) {
  ...
  if (autoDistributeAmt !== 'N' && autoDistributeAmt !== '"N"') {
    ...
    OB.EFIN.AddPayment.updateActualExpected(form);
    OB.EFIN.AddPayment.updateInvOrderTotal(form, orderInvoice);
  }
};

OB.EFIN.AddPayment.updateTotal = function (form) {
  ...
  OB.EFIN.AddPayment.updateDifference(form);
};

OB.EFIN.AddPayment.updateDifference = function (form) {
  ...
  if (diffAmt.signum() !== 0) {
    OB.EFIN.AddPayment.updateDifferenceActions(form);
  }
  affectedParams.push(form.getField('overpayment_action_display_logic').paramId);
  OB.EFIN.AddPayment.recalcDisplayLogicOrReadOnlyLogic(form, form.paramWindow, affectedParams);
};

OB.EFIN.AddPayment.updateDifferenceActions = function (form) {
  var issotrx = form.getItem('issotrx').getValue(),
      overpaymentAction = form.getItem('overpayment_action'),
      actualPayment = new BigDecimal(String(form.getItem('actual_payment').getValue() || 0)),
      newValueMap = {},
...
OB.EFIN.AddPayment.updateInvOrderTotal = function (form, grid) {
...
OB.EFIN.AddPayment.selectionChanged = function (record, state) {
  ...
  if (!orderInvoice.preventDistributingOnSelectionChanged) {
    this.fireOnPause('selectionChanged' + record.id, function () {
      OB.EFIN.AddPayment.doSelectionChanged(record, state, this.view);
    }, 200);
    this.Super('selectionChanged', arguments);
  }
};

OB.EFIN.AddPayment.doSelectionChanged = function (record, state, view) {
  ...
  if (!orderInvoice.obaprmAllRecordsSelectedByUser || (orderInvoice.obaprmAllRecordsSelectedByUser && (orderInvoice.getRecordIndex(record) === orderInvoice.getTotalRows() - 1))) {
    ...
    OB.EFIN.AddPayment.updateInvOrderTotal(view.theForm, orderInvoice);
    OB.EFIN.AddPayment.updateActualExpected(view.theForm);
    OB.EFIN.AddPayment.updateDifference(view.theForm);
...
OB.EFIN.AddPayment.userSelectAllRecords = function () {
  this.obaprmAllRecordsSelectedByUser = true;
  this.Super('userSelectAllRecords', arguments);
};

OB.EFIN.AddPayment.deselectAllRecords = function () {
  this.obaprmAllRecordsSelectedByUser = true;
  this.Super('deselectAllRecords', arguments);
};


OB.EFIN.AddPayment.updateActualExpected = function (form) {
  ...
  if (!issotrx) {
    ...
    OB.EFIN.AddPayment.updateDifference(form);
    OB.EFIN.AddPayment.updateConvertedAmount(null, form, false);
  }
...
OB.EFIN.AddPayment.removeRecordClick = function (rowNum, record) {
  this.Super('removeRecordClick', rowNum, record);

  OB.EFIN.AddPayment.updateGLItemsTotal(this.view.theForm, rowNum, true);
};

OB.EFIN.AddPayment.updateGLItemsTotal = function (form, rowNum, remove) {
  var totalAmt = BigDecimal.prototype.ZERO,
  ...
  OB.EFIN.AddPayment.updateTotal(form);
  affectedParams.push(form.getField('overpayment_action_display_logic').paramId);
  OB.EFIN.AddPayment.recalcDisplayLogicOrReadOnlyLogic(form, grid.view, affectedParams);
  return true;
};

OB.EFIN.AddPayment.glItemAmountOnChange = function (item, view, form, grid) {
  ...
  OB.EFIN.AddPayment.updateGLItemsTotal(form, item.rowNum, false);
  OB.EFIN.AddPayment.updateActualExpected(form);
  OB.EFIN.AddPayment.updateDifference(form);
  return true;
};

OB.EFIN.AddPayment.updateCreditTotal = function (form) {
  var totalAmt = BigDecimal.prototype.ZERO,
...
OB.EFIN.AddPayment.updateCreditOnChange = function (item, view, form, grid) {
  var issotrx = form.getItem('issotrx').getValue();

  OB.EFIN.AddPayment.updateCreditTotal(form);
  if (issotrx) {
    OB.EFIN.AddPayment.distributeAmount(view, form, true);
  }
  OB.EFIN.AddPayment.updateDifference(form);
...
OB.EFIN.AddPayment.selectionChangedCredit = function (record, state) {
  ...
  if (!creditgrid.preventDistributingOnSelectionChanged) {
    this.fireOnPause('selectionChangedCredit' + record.id, function () {
      OB.EFIN.AddPayment.doSelectionChangedCredit(record, state, this.view);
    }, 200);
    this.Super('selectionChanged', arguments);
  }
};

OB.EFIN.AddPayment.orderInvoiceGridValidation = function (item, validator, value, record) {
...
OB.EFIN.AddPayment.creditValidation = function (item, validator, value, record) {
  var outstanding = new BigDecimal(String(record.outstandingAmount)),
...
OB.EFIN.AddPayment.doSelectionChangedCredit = function (record, state, view) {
  ...
  OB.EFIN.AddPayment.updateCreditTotal(view.theForm);
  OB.EFIN.AddPayment.updateActualExpected(view.theForm);
  if (issotrx) {
    OB.EFIN.AddPayment.distributeAmount(view, view.theForm, true);
  }
};

OB.EFIN.AddPayment.conversionRateOnChange = function (item, view, form, grid) {
  OB.EFIN.AddPayment.updateConvertedAmount(view, form, false);
};

OB.EFIN.AddPayment.convertedAmountOnChange = function (item, view, form, grid) {
  OB.EFIN.AddPayment.updateConvertedAmount(view, form, true);
};

OB.EFIN.AddPayment.updateConvertedAmount = function (view, form, recalcExchangeRate) {
 ...
 */
OB.EFIN.AddPayment.orderAndRemoveDuplicates = function (val) {
...
OB.EFIN.AddPayment.documentOnChange = function (item, view, form, grid) {
...
OB.EFIN.AddPayment.organizationOnChange = function (item, view, form, grid) {
  var ordinvgrid = form.getItem('order_invoice').canvas.viewGrid,
...
OB.EFIN.AddPayment.receivedFromOnChange = function (item, view, form, grid) {
  var affectedParams = [],
      trxtype = (form.getItem('trxtype')) ? form.getItem('trxtype').getValue() : "",
  ...
  callback = function (response, data, request) {
    if (data.paymentMethodId !== '') {
      ...
      form.redraw();
      OB.EFIN.AddPayment.paymentMethodOnChange(paymentMethodItem, view, form, grid);
    }
  };
...
OB.EFIN.AddPayment.recalcDisplayLogicOrReadOnlyLogic = function (form, view, affectedParams) {
...
OB.EFIN.AddPayment.reloadLabels = function (form) {
...
OB.EFIN.AddPayment.onProcess = function (view, actionHandlerCall, clientSideValidationFail) {
