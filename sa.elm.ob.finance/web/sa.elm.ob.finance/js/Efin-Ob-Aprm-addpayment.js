...
OB.APRM.AddPayment.onLoad = function(view) {
    ...
    var paymentDate = form.getItem('payment_date').getValue();
    var paymentHijriDate = calendar.newDate().fromJSDate(paymentDate),
        formattedDate = calendar.formatDate('dd-mm-yyyy', paymentHijriDate);
    form.getItem('payment_date').setValue(formattedDate + "");
...
OB.APRM.AddPayment.orderInvoiceOnLoadGrid = function(grid) {
    ...
    if ((issotrx || !payment) && (grid.selectedIds.length === 0)) {
        ...
        if (!issotrx) {
        	var form =this.view.theForm;
            callback = function(response, data, request) {
                if (data.isInvoice) {
                    grid.selectAllRecords();
                }
                OB.APRM.AddPayment.updateInvOrderTotal(form, grid);
                OB.APRM.AddPayment.tryToUpdateActualExpected(form);
            };
            OB.RemoteCallManager.call('sa.elm.ob.finance.actionHandler.PaymentOutInvoiceActionHandler', {}, param, callback);
        }
    }
};
...
OB.APRM.AddPayment.distributeAmount = function(view, form, onActualPaymentChange) {
    ...
    if (autoDistributeAmt !== 'N' && autoDistributeAmt !== '"N"') {
        ...
        OB.APRM.AddPayment.updateconvertedvalue(view, form, orderInvoice);
...
/*if (amount.signum() !== 0 && state) {
    ...
    if (!orderInvoice.obaprmAllRecordsSelectedByUser || (orderInvoice.obaprmAllRecordsSelectedByUser && (orderInvoice.getRecordIndex(record) === orderInvoice.getTotalRows() - 1))) {
        ...
        OB.APRM.AddPayment.updateconvertedvalue(view, view.theForm, orderInvoice);
...
OB.APRM.AddPayment.updateconvertedvalue = function(view, form, grid) {

    var orderInvoiceGrid = form.getItem('order_invoice').canvas.viewGrid,
        receivedFrom = form.getItem('received_from').getValue(),
        issotrx = form.getItem('issotrx').getValue(),
        payment = form.getItem('fin_payment_id').getValue(),
        finFinancialAccount = form.getItem('fin_financial_account_id').getValue(),
        selectedRecords = orderInvoiceGrid.getSelectedRecords(),
        actualPayment = new BigDecimal(
        String(form.getItem('actual_payment').getValue())),
        currencyPrecision = form.getItem('StdPrecision').getValue(),
        newConvertedactualAmount = BigDecimal.prototype.ZERO;

    callbackOnProcessActionHandler = function(response, data, request) {
        if (data.message!==undefined && data.message.severity === 'error') {
            view.messageBar.setMessage(isc.OBMessageBar.TYPE_ERROR, data.message.title, data.message.text);
            form.getItem('conversion_rate').setValue("");
            form.getItem('converted_amount').setValue("");
            return false;
        } else {
            form.getItem('conversion_rate').setValue(data.conversionrate);
        }
    ...
    OB.RemoteCallManager.call('org.openbravo.actionHandler.AddPaymentOnProcessConversionActionHandler', {
        payment: payment,
        issotrx: issotrx,
...
OB.APRM.AddPayment.updateActualExpected = function(form) {
    ...
    if (selectedRecords.length > 0) {
        ...
        actualPayment.setValue(Number(totalAmountoutstanding));
    } else {
