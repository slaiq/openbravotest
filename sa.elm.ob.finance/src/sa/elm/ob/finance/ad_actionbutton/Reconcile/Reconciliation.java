...
package sa.elm.ob.finance.ad_actionbutton.Reconcile;
...
public class Reconciliation extends HttpSecureAppServlet {
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (vars.commandIn("DEFAULT")) {
      ...
      printGrid(response, strFinancialAccountId, strStatementDate, strAfterDate,
          ...
          strSortColumn, strSortType, strValidMoF);
      ...
      String[] strSelectedTransId = null;
      String strFinancialAccountId = vars.getStringParameter("inpFinFinancialAccountId", "");
      String strSelectedTransIdList = vars.getStringParameter("inpCurrentTransIdSelected", "");

      if (strSelectedTransIdList != null && strSelectedTransIdList != "")
        strSelectedTransId = strSelectedTransIdList.split(",");
  ...
  private void updateTransactionStatus(HttpServletResponse response, String strFinancialAccountId,
    ...
    try {
      if (strSelectedTransId != null && strSelectedTransId.length > 0) {

        FIN_FinancialAccount account = OBDal.getInstance().get(FIN_FinancialAccount.class,
            strFinancialAccountId);
        FIN_Reconciliation reconciliation = null;
        if (isChecked) {
          reconciliation = TransactionsDao.getLastReconciliation(account, "N");
        }
        for (int i = 0; i < strSelectedTransId.length; i++) {
          FIN_FinaccTransaction trans = OBDal.getInstance().get(FIN_FinaccTransaction.class,
              strSelectedTransId[i]);
          ...
          if (!isChecked) {
            ...
            if (trans.getFinPayment() != null) {
              trans.getFinPayment()
                  .setStatus((trans.getFinPayment().isReceipt()) ? "RDNC" : "PWNC");
            }
          } else {
  ...
  private void processReconciliation(HttpServletResponse response, VariablesSecureApp vars,
    ...
    try {
      ...
      if (process) { // Validations
        ...
        for (FIN_FinaccTransaction finacctrxv : reconciliation.getFINFinaccTransactionList()) {
          if (reconciliation.getEndingDate().compareTo(finacctrxv.getTransactionDate()) < 0) {// finacctrxv.getFinancialAccountTransaction()
          ...
          if (finacctrxv.getFinPayment() != null) {
            for (FIN_PaymentDetail pd : finacctrxv.getFinPayment().getFINPaymentDetailList()) {// finacctrxv.getPayment()
              for (FIN_PaymentScheduleDetail psd : pd.getFINPaymentScheduleDetailList()) {
                invoicePaidold = psd.isInvoicePaid();
                if (!invoicePaidold) {
                  if ((FIN_Utility
                      .invoicePaymentStatus(finacctrxv.getFinPayment().getPaymentMethod(),
                          reconciliation.getAccount(), finacctrxv.getFinPayment().isReceipt())
                      .equals(finacctrxv.getFinPayment().getStatus()))) {
              ...
              FIN_Utility.updateBusinessPartnerCredit(finacctrxv.getFinPayment());
            }
          }
        }
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strOrgId,
    ...
    try {
      ...
      XmlDocument xmlDocument = xmlEngine
          .readXmlTemplate("sa/elm/ob/finance/ad_actionbutton/Reconcile/Reconciliation")
          .createXmlDocument();
  ...
  private void printGrid(HttpServletResponse response, String strFinancialAccountId,
      ...
      String strClearedAmt, String strTotPayment, String strTotDeposit, String sortColumn,
    ...
    String strCurrentlyCleared = strClearedAmt;
    String strTotalPayment = strTotPayment;
    String strTotalDeposit = strTotDeposit;
    XmlDocument xmlDocument = xmlEngine
        .readXmlTemplate("sa/elm/ob/finance/ad_actionbutton/Reconcile/ReconciliationGrid")
        .createXmlDocument();
    ...
    if (strCurrentlyCleared == null || strCurrentlyCleared.equals("")) {
      strCurrentlyCleared = "0";
    }
    if (strTotalPayment == null || strTotalPayment.equals("")) {
      strTotalPayment = "0";
    }
    if (strTotalDeposit == null || strTotalDeposit.equals("")) {
      strTotalDeposit = "0";
    }
  ...
  public boolean getDocumentConfirmation(ConnectionProvider conn, String strRecordId) {
    ...
    try {
      FIN_Reconciliation reconciliation = OBDal.getInstance().get(FIN_Reconciliation.class,
          strRecordId);
      List<FIN_FinaccTransaction> transactions = reconciliation.getFINFinaccTransactionList(); // getTransactionList(reconciliation);
      ...
      for (FIN_FinaccTransaction transaction : transactions) {
        ...
        if (payment != null) {
          OBQuery<FinAccPaymentMethod> accPayMethod = OBDal.getInstance().createQuery(
              FinAccPaymentMethod.class,
              " as e where e.account=:account and e.paymentMethod=:paymentmethod ");
          accPayMethod.setNamedParameter("account", reconciliation.getAccount());
          accPayMethod.setNamedParameter("paymentmethod", payment.getPaymentMethod());
          accPayMethod.setFilterOnReadableClients(false);
          accPayMethod.setFilterOnReadableOrganization(false);
          ...
          List<FinAccPaymentMethod> lines = accPayMethod.list();
