...
public class TransactionsDao {
  ...
  public static FieldProvider[] getTransactionsFiltered(FIN_FinancialAccount account,
    ...
    try {
      ...
      whereClause.append(
          " select to_char(ft.transactionDate,'dd-MM-yyyy') , subString(trx. hijriDate,7,2) ||'-'|| subString(trx. hijriDate,5,2)|| '-'|| subString(trx. hijriDate,0,5) "
              + ", to_char(pym.efinMofchequedate,'dd-MM-yyyy'),subString(mof. hijriDate,7,2) ||'-'|| subString(mof. hijriDate,5,2)|| '-'|| subString(mof. hijriDate,0,5) from FIN_Finacc_Transaction as ft");
      whereClause.append(" left outer join ft.reconciliation as rec");
      whereClause
          .append(" left outer join ft.finPayment as pym,EUT_HijiriDates trx,EUT_HijiriDates mof");
      whereClause.append(" where ft.account.id = ?");
      whereClause.append(" and (rec is null or rec.processed = 'N')");
      whereClause.append(" and ft.processed = 'Y'");
      whereClause.append(" and ft.processed = 'Y'");
      ...
      whereClause.append(
          " and pym.receipt = 'N' and trx.gregorianDate=ft.transactionDate and pym.efinMofchequedate=mof.gregorianDate  and ft.client.id= trx.client.id\n"
              + " and pym.client.id= mof.client.id ");
      if (strValidMoF.equals("Y")) {
        ...
        whereClause.append(" and (pym.efinMofchequedate is not null ) ");
        whereClause
      ...
      final Query dateQuery = OBDal.getInstance().getSession().createQuery(whereClause.toString());
      dateQuery.setParameter(0, account.getId());
      if (hideAfterDate) {
        dateQuery.setParameter(1, statementDate);
      }

      if (dateQuery != null && dateQuery.list().size() > 0) {
        List<Object> dateList = dateQuery.list();
        for (Object obj : dateList) {
          Object[] row = (Object[]) obj;
          DateMap.put(row[0].toString(), row[1].toString());
          DateMap.put(row[2].toString(), row[3].toString());
        }
      }
      ...
      whereClause.append(" and ft.status<>'EFIN_CAN'");
      whereClause.append(" and ft.posted = 'Y'");
      whereClause.append(" and pym.receipt = 'N'");
      if (strValidMoF.equals("Y")) {
        whereClause.append(" and (pym.efinMofbankname is not null and pym.efinMofbankname <> '') ");
        whereClause.append(" and (pym.efinMofchequeno is not null and pym.efinMofchequeno <>'')  ");
        whereClause.append(" and (pym.efinMofchequedate is not null ) ");
      ...
      if (sortColumn.isEmpty())
        whereClause.append(" order by pym.efinMofchequeno asc ");
      else if (sortColumn.equals("Date"))
        whereClause.append(" order by ft.transactionDate,pym.efinMofchequeno ");
        ...
        whereClause.append(" order by pym.efinMofchequedate,pym.efinMofchequeno ");
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        if (DateMap != null && DateMap.containsKey(trxDate)) {
          trxDate = DateMap.get(trxDate);
        }
        ...
        if (FIN_Transactions[i].getFinPayment().getEfinMofchequedate() != null
            && !FIN_Transactions[i].getFinPayment().getEfinMofchequedate().equals("")) {
          ...
          if (DateMap != null && DateMap.containsKey(MofChequeDate)) {
            MofChequeDate = DateMap.get(MofChequeDate);
          }
