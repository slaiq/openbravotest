...
public class AdvPaymentMngtDao {
  ...
  public FIN_Payment getNewPayment(boolean isReceipt, Organization organization,
      ...
      Date paymentDate, String referenceNo, Currency currency, BigDecimal conversionRate,
      BigDecimal convertedAmt, EutNextRole nextRole, JSONObject jsonparams, String strInvoiceId) {
  ...
  public FIN_Payment getNewPayment(boolean isReceipt, Organization organization,
      ...
      BigDecimal finTxnAmount, JSONObject jsonParams, String strInvoiceID) {
    try {
      ...
      if (isReceipt) {
        final String strCustomerName = jsonParams.getString("efin_cusname") == null ? ""
            : jsonParams.getString("efin_cusname");
        final String strCustomerNo = jsonParams.getString("efin_number") == null ? ""
            : jsonParams.getString("efin_number");
        final String strCustomerLoc = jsonParams.getString("efin_location") == null ? ""
            : jsonParams.getString("efin_location");

        newPayment.setEFINName(strCustomerName);
        newPayment.setEFINNumber(strCustomerNo);
        newPayment.setEFINLocation(strCustomerLoc);

        if (strInvoiceID.length() == 32) {
          Invoice invoice = sa.elm.ob.utility.util.Utility.getObject(Invoice.class, strInvoiceID);

          invoice.setEfinCusname(strCustomerName);
          invoice.setEfinCusno(strCustomerNo);
          invoice.setEfinCuslocation(strCustomerLoc);
  ...
  public FIN_Payment getNewPayment(boolean isReceipt, Organization organization,
      ...
      FIN_PaymentMethod paymentMethod, FIN_FinancialAccount finAccount, String strPaymentAmount,
      Date paymentDate, String referenceNo, Currency paymentCurrency, BigDecimal finTxnConvertRate,
      BigDecimal finTxnAmount, EutNextRole nextRole, JSONObject jsonParams) {
    try {
      ...
      if (isReceipt)
        ...
        newPayment.setStatus(nextRole == null ? "RPAP" : "EFIN_WFA");
      ...
      newPayment.setFinancialTransactionAmount(finTxnAmount);
      if (isReceipt)
        newPayment.setEutNextRole(null);
      else
        newPayment.setEutNextRole(nextRole);
      try {
        newPayment.setEfinDocumentAction(jsonParams.getString("document_action"));
  ...
  public FIN_FinaccTransaction getNewFinancialTransaction(Organization organization,
    ...
    if (payment != null) {
      ...
      /*
       * if(payment != null && payment.getQmrChequeno() != null) {
       * finTrans.setQmlChequeno(payment.getQmrChequeno()); }
       */
    }
  ...
  public FieldProvider[] getPaymentsNotDeposited(FIN_FinancialAccount account, Date fromDate,
    ...
    try {
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        // FieldProviderFactory.setField(data[i], "paymentChequeNo",
        // FIN_Payments[i].getQmrChequeno());
  ...
  public List<FIN_Payment> getCustomerPaymentsWithCredit(Organization org, BusinessPartner bp,
    ...
    try {
      ...
      for (FIN_Payment fp : obcPayment.list()) {
        if (!fp.getStatus().equals("EFIN_WFA")) {
