package sa.elm.ob.finance.properties;
	...
	private static String getValue(String propKey, String lang, boolean escape) {
		...
		if(resource == null) {
			...
			try {
				...
				InputStream input = Thread.currentThread().getContextClassLoader().getResourceAsStream("sa/elm/ob/finance/properties/applicationresources.properties");
		...
		if(prop != null) {
			...
			try {
				...
				InputStream input = Thread.currentThread().getContextClassLoader().getResourceAsStream("sa/elm/ob/finance/properties/applicationresources_" + lang.toLowerCase() + ".properties");
