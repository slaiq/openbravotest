...
public class SL_Invoice_DocType extends HttpSecureAppServlet {
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars,
    ...
    if (data == null || data.length == 0)
      ...
      // set encumbrance type based on document type selected in purchase invoice
      if (StringUtils.isNotEmpty(strDocTypeTarget)) {
        DocumentType doctype = OBDal.getInstance().get(DocumentType.class, strDocTypeTarget);
        if (doctype != null) {
          if (doctype.getDocumentCategory().equals("API") && (!doctype.isEfinIsprepayinv())
              && (!doctype.isEfinIsprepayinvapp()) && (!doctype.isEfinIspomatch())) {
            resultado.append(", new Array(\"inpemEfinEncumType\", \"AEE\")");
            resultado.append(", new Array(\"inpemEfinInvoicetypeTxt\", \"API\")");
          } else if (doctype.getDocumentCategory().equals("API")
              && (doctype.isEfinIsprepayinv() || doctype.isEfinIsprepayinvapp())) {
            resultado.append(", new Array(\"inpemEfinEncumType\", \"AAE\")");
            if (doctype.isEfinIsprepayinv()) {
              resultado.append(", new Array(\"inpemEfinInvoicetypeTxt\", \"PPI\")");
            }

            if (doctype.isEfinIsprepayinvapp()) {
              resultado.append(", new Array(\"inpemEfinInvoicetypeTxt\", \"PPA\")");
            }
          } else if (doctype.getDocumentCategory().equals("API") && (doctype.isEfinIspomatch())) {
            resultado.append(", new Array(\"inpemEfinEncumType\", \"POE\")");
            resultado.append(", new Array(\"inpemEfinInvoicetypeTxt\", \"POM\")");
          }
        }
      }
      resultado.append(");");
