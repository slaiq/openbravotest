...
public class SE_Invoice_BPartner extends HttpSecureAppServlet {
  ...
  private void printPage(HttpServletResponse response, VariablesSecureApp vars, String strBPartner,
    ...
    if (data == null || data.length == 0)
      ...
      resultado.append("new Array(\"inpemEfinIdno\", \"" + null + "\"),");

      resultado.append("new Array(\"inpemEfinCustomeridtype\", \"" + null + "\"),");
      resultado.append("new Array(\"inpemEfinCustomertype\", \""
          + ((bPartner != null && bPartner.getEFINCustomerType() != null)
              ? bPartner.getEFINCustomerType().getId()
              : null)
          + "\"),");
      ...
      if (FIN_Utility.isBlockedBusinessPartner(strBPartner, "Y".equals(strIsSOTrx), 3)) {
        ...
        bPartner = OBDal.getInstance().get(BusinessPartner.class, strBPartner);
        if (message.length() > 0) {
          message = message + "<br>";
        }
