  ...
  var buttonProps = {
    action: function () {
      ...
      var callback = function (ok) {
          ...
          if (ok) {
            requestParams = {
              ProposalId: view.viewGrid.getSelectedRecord().id,
              Type:"PRO"
      ...
      var EscmCallback = function (response, data, request) {
          if (typeof view.viewGrid.getSelectedRecord() != 'undefined') {
        	  if ((view.viewGrid.getSelectedRecord().eFINUniqueCode != null || data.Message === "Success") && data.isBudgetContrl === "Yes" ) { 	
        		  ...
        		  if (view.viewGrid.getSelectedRecord().proposalappstatus == 'APP' || view.viewGrid.getSelectedRecord().efinIsbudgetcntlapp == true)  {
        			  disable = true;
              } 
              else 
            	  {
    ...
    buttonType: 'escm_proposal_apply_uniquecode',
    prompt: OB.I18N.getLabel('escm_pr_distribute_all'),
  };

  // register the button for the proposal mgmt tab
  // the first parameter is a unique identification so that one button can not be registered multiple times.
  OB.ToolbarRegistry.registerButton(buttonProps.buttonType, isc.OBToolbarIconButton, buttonProps, 120, 'D6115C9AF1DD4C4C9811D2A69E42878B');
}());