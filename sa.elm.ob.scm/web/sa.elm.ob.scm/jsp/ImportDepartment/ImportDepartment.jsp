 ...
 <%@page import="sa.elm.ob.scm.ad_process.ImportDepartment.vo.ImportDepartmentVO"%>
...
<%@page import="java.util.List, java.util.ArrayList"%>
 <%     
    String lang = ((String)session.getAttribute("#AD_LANGUAGE"));
     ...
     List<ImportDepartmentVO> orglist = (ArrayList<ImportDepartmentVO>) request.getAttribute("orglist");
 %>
<HTML>
<HEAD>
<META http-equiv="Content-Type" content="text/html; charset=utf-8"></META>
<TITLE><%=Resource.getProperty("scm.import.department.title",lang)%></TITLE>
  ...
  <span class="tabTitle">
    <div class="tabTitle_background">
      <span class="tabTitle_elements_container">
        <span class="tabTitle_elements_text" id="tabTitle_text"><%=Resource.getProperty("scm.import.department.title",lang)%></span>
   ...
   <div>
     <span class="dojoTabcurrentfirst">
        <div>
           ...
           <%=Resource.getProperty("scm.import.department.title",lang)%>
            </a></span>
         </div>
    </span>
   </div>
        ...
        <DIV class="Main_ContentPane_Client" style="overflow: auto;" id="client">
              ...
              <TABLE class="Main_Client_TableEdition">
                ...
                <TR>
                <TD></TD>
                    <TD class="TitleCell"><SPAN class="LabelText"><%=Resource.getProperty("scm.organization",lang)%></SPAN></TD>
            
                     <TD class="TextBox_ContentCell">
                                           <SELECT id="inpOrgId" name="inpOrgId"   class="ComboKey Combo_TwoCells_width" onchange="document.getElementById('messageBoxID').style.display = 'none';" onkeydown="return true;" onkeyup="enableForm();">
                                               <%  if(orglist!=null && orglist.size()>0) { %>
                                                <option value="00"><%= Resource.getProperty("scm.--select--", lang) %></option>
                                                <% for(ImportDepartmentVO vo:orglist){
                                                     %>
...
function validateFile() {
    ...
    if(validateFileExtension()==false)
        return false;
    if(document.getElementById("inpOrgId").value=='00')
    {
  OBAlert('<%= Resource.getProperty("scm.import.department.select.org", lang) %>');
   return false;
    }
    document.forms['frmMain'].action= "<%=request.getContextPath() %>/sa.elm.ob.scm.ad_process.ImportDepartment.header/ImportDepartment?action=validateCsv&inpOrgId="+document.getElementById('inpOrgId').value;
...
function uploadCSV() {
    ...
    document.forms['frmMain'].action= "<%=request.getContextPath() %>/sa.elm.ob.scm.ad_process.ImportDepartment.header/ImportDepartment?action=uploadCsv&inpOrgId="+document.getElementById('inpOrgId').value;
    document.forms['frmMain'].submit();
