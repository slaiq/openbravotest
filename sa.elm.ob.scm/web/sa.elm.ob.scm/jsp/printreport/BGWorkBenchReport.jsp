      ...
      <TR>
        <TD>
          ...
          <TABLE cellspacing="0" cellpadding="0" class="Popup_Client_TablePopup">           
            ...
            <table>   
            <tr><td></td></tr>
            <tr id="bgworkbench_report" style="display: none;">
           <td colspan="2"><table>
                <tr id="bgworkbench_report_select">
                    <TD class="TitleCell" align="left"><SPAN class="LabelText"> <%=Resource.getProperty("scm.BGW.BGWorkBenchReport",lang)%></SPAN></TD>
                    <TD class="TextBox_btn_ContentCell TextBox_ReadOnly_ContentCell">
                        ...
                        <SELECT name="inpBGWorkbenchRpt" class="ComboKey Combo_TwoCells_width" id="inpBGWorkbenchRpt" >
                            <option value="0"><%= Resource.getProperty("scm.--select--", lang) %></option>
                            <option value="BGVLR"><%= Resource.getProperty("scm.BGW.BGVerficationLetterReport", lang) %></option> 
                            <option value="BGELR"><%= Resource.getProperty("scm.BGW.BGExtentionLetterreport",lang)%></option>    
                           <option value="BGCLR"><%= Resource.getProperty("scm.BGW.BGConfiscationLetterReport",lang)%></option>
                            <option value="BGRLR"><%= Resource.getProperty("scm.BGW.bgreleaseletterreport",lang)%></option>
                            <option value="BGDR"><%= Resource.getProperty("scm.BGW.BGDetailsReport",lang)%></option>                 
                            <option value="BGARLR"><%= Resource.getProperty("scm.BGW.bgamountreductionletterreport",lang)%></option>                         
                        </SELECT>
                        </TD></TR></TABLE>
                    </TD>
                </tr>
...
function GenerateReport(){
    ...
    if($('#inpWindowID').val()=='6AB9C67CA449404D803E35543BD4C093')  { //BG Workbench 
        
        if($("#inpBGWorkbenchRpt").val()=='0'){
             OBAlert("<%= Resource.getProperty("scm.pm.pleaseselectonereport",lang)%>");
             return false;
         }
        submitCommandForm('DEFAULT', false, null, '<%=request.getContextPath()%>/sa.elm.ob.scm.ad_process.printreport/PrintReport?report=BankGuaranteeworkbench', 'background_target', null, false);        
    }    
    closePage();
    return true;
}
