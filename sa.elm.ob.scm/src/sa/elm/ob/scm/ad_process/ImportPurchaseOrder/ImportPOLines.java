...
public class ImportPOLines extends HttpSecureAppServlet {
  ...
  private static final String includeIn = "../web/sa.elm.ob.scm/jsp/ImportPurchaseOrder/ImportPO.jsp";
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (inpAction.equals("")) {
      ...
      String inpOrderId = vars.getStringParameter("inpRecordId");
      request.setAttribute("inpOrderId", inpOrderId);
    } else if (inpAction.equals("uploadCsv") || inpAction.equals("validateCsv")) {
      String inpOrderId = request.getParameter("inpOrderId");
      JSONObject isValidCSV = null;
      try {
        // parses the request's content to extract file data

        ImportPOLinesDAO dao = new ImportPOLinesDAO(con);
        ...
        if (isMultipart) {
          ...
          if (inpAction.equals("validateCsv")) {
            JSONObject resultJSON = dao.processValidateCsvFile(attachmentFile, vars, inpOrderId,
                false);
            if (resultJSON != null) {
            ...
            if (resultJSON != null) {
              if ("1".equals(resultJSON.getString("status"))) {
                isValidCSV = dao.processUploadedCsvFile(inpOrderId, vars);
