...
public class OpenEnvelopeEventImport extends HttpSecureAppServlet {

  /**
   * Servlet implementation class to import PO Lines
   */
  ...
  private static final String includeIn = "../web/sa.elm.ob.scm/jsp/importopenenvelope/ImportOpenEnvelopeEvent.jsp";
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (inpAction.equals("")) {
      ...
      String inpOpenEnvelopeEventId = vars.getStringParameter("inpRecordId");
      request.setAttribute("inpOpenEnvelopeEventId", inpOpenEnvelopeEventId);
    } else if (inpAction.equals("uploadCsv") || inpAction.equals("validateCsv")) {
      String inpOpenEnvelopeEventId = request.getParameter("inpOeeId");
      JSONObject isValidCSV = null;
      try {
        // parses the request's content to extract file data

        OpenEnvelopeEventImportDAO dao = new OpenEnvelopeEventImportDAO(con);
        ...
        if (isMultipart) {
          ...
          try {
            ...
            log4j.error("Exception while uploading the OpenEnvelopeEvent Excel file:" + e);
          ...
          if (inpAction.equals("validateCsv")) {
            JSONObject resultJSON = dao.processValidateCsvFile(attachmentFile, vars,
                inpOpenEnvelopeEventId, false);
            if (resultJSON != null) {
            ...
            JSONObject resultJSON = dao.processValidateCsvFile(attachmentFile, vars,
                inpOpenEnvelopeEventId, true);
            if (resultJSON != null) {
              if ("1".equals(resultJSON.getString("status"))) {
                isValidCSV = dao.processUploadedCsvFile(inpOpenEnvelopeEventId, vars);
