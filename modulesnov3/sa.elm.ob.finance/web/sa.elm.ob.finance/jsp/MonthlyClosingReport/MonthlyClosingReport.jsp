...
<HTML xmlns="http://www.w3.org/1999/xhtml">
    ...
    <TITLE><%= Resource.getProperty("hcm.monthlyClosing",lang) %></TITLE>
                                        ...
                                        <div>
                                            <span class="dojoTabcurrentfirst">
                                                <div>
                                                    ...
                                                    <%= Resource.getProperty("hcm.monthlyClosing",lang).replace("'", "\\\'") %>
                                                    </a></span>
                                                </div>
                                            </span>
                                        </div>
                                                    ...
                                                    <TABLE  align="center" width="100%">
                                                        ...
                                                        <tr>
                                                            <td class="TitleCell"><span
                                                              class="LabelText" id="inpGeneralLabel"> <b> <%=Resource.getProperty("hcm.generalledger", lang)
                                                             .replace("'", "\\\'")%>
                                                             </b>
                                                            </span>
                                                            </td>
                                                            <td class="TextBox_ContentCell">
                                                                   <select id="inpAcctSchemaId" class="ComboKey Combo_TwoCells_width">
                                                                   </select>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td class="TitleCell"><span class="LabelText" id="inpYearLabel"> <b> <%=Resource.getProperty("hcm.fiscalCalendar", lang).replace("'", "\\\'")%>
                                                             </b>
                                                            </span>
                                                            </td>
                                                            <td class="TextBox_ContentCell">
                                                                   <select id="inpYearId" class="ComboKey Combo_TwoCells_width" onchange="generatePeriod(this.value)">
                                                                   </select>
                                                            </td>
                                                        </tr>
                                                          <tr>
                                                            <td class="TitleCell"><span class="LabelText" id="inpPeriodLabel"> <b> <%=Resource.getProperty("hcm.toperiod", lang).replace("'", "\\\'")%>
                                                             </b>
                                                            </span>
                                                            </td>
                                                            <td class="TextBox_ContentCell">
                                                                   <select id="inpPeriodId" class="ComboKey Combo_TwoCells_width">
                                                                    ...
                                                                    <BUTTON type="button" id="Submit_linkBTN" class="ButtonLink" onclick="generateReport();" style="margin: 10px 0;">
                                                                        <TABLE class="Button">
                                                                            <TR>
                                                                                ...
                                                                                <%= Resource.getProperty("hcm.generateReport",lang).replace("'", "\\\'") %>
                                                                                </TD>
    ...
    <script type="text/javascript" src="../web/js/ui/jquery.ui.autocomplete.js"></script>   
        <script type="text/javascript">
  //load general Ledger       
        var url = "<%=request.getContextPath()%>/MonthlyClosingReportAjax?action=getGeneralLedger";
        $.getJSON(url, function(result){
            ...
            $("#inpAcctSchemaId").append("<option value='0' selected>"+"--"+select+"--"+"</option>");
            $.each( data, function( i, item ) {
                $("#inpAcctSchemaId").append("<option value='"+item.schemaId+"'>"+item.schemaName+"</option>");
              });
        });
   //load year 
     var url = "<%=request.getContextPath()%>/MonthlyClosingReportAjax?action=getYear";
        $.getJSON(url, function(result){
            ...
            var select = "<%=Resource.getProperty("hcm.select", lang)%>";
            $("#inpYearId").append("<option value='0' selected>"+"--"+select+"--"+"</option>");
            $.each( data, function( i, item ) {
                $("#inpYearId").append("<option value='"+item.id+"'>"+item.name+"</option>");
              });
        });
        
        function generatePeriod(yearId){
            //load Job No
            var url = "<%=request.getContextPath()%>/MonthlyClosingReportAjax?action=getPeriod&yearId="+yearId;
                $.getJSON(url, function(result){
                    ...
                    .end();
                    var select = "<%=Resource.getProperty("hcm.select", lang)%>";
                    $("#inpPeriodId").append("<option value='0' selected>"+"--"+select+"--"+"</option>");
                    $.each( data, function( i, item ) {
                        $("#inpPeriodId").append("<option value='"+item.id+"'>"+item.name+"</option>");
                      });
                });
        }
    function reloadWindow() 
    {
        submitCommandForm('DEFAULT', false, null, 'MonthlyClosingReport', '_self', null, false);
        return false;
    }
...
function generateReport(){
      var acctSchemaId=document.getElementById("inpAcctSchemaId").value, yearId=document.getElementById("inpYearId").value,inpPeriodId=document.getElementById("inpPeriodId").value;
      if(acctSchemaId == 0 || yearId == 0 || inpPeriodId == 0 ){
          displayMessage("W","Warning", '<%=Resource.getProperty("finance.Nodatafound", lang)%>');
      }else{
         document.getElementById('messageBoxID').style.display='none';
      document.forms['frmMain'].action= '<%=request.getContextPath() %>/sa.elm.ob.finance.reports.MonthlyClosingReport/MonthlyClosingReport?inpAction=DownloadReport&inpAcctSchemaId='+acctSchemaId+'&inpYearId='+yearId+'&inpPeriodId='+inpPeriodId;
