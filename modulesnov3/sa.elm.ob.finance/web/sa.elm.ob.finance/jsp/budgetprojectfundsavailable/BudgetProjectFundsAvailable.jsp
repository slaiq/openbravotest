...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    /* JSONArray budAccts = (JSONArray) request.getAttribute("inpBudgetAccounts"); */
...
<HEAD>
<TITLE><%= Resource.getProperty("finance.budgetprojectfundsavailable.title",lang) %></TITLE>
...
function onResizeDo() {
   ...
   <BODY onresize="onResizeDo();" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" id="paramBody">
      <FORM id="form" method="post" action="" name="frmMain">
         ...
         <INPUT type="hidden" name="inpFromAccount" id="inpFromAccount" value=""></INPUT>
          <INPUT type="hidden" name="inpToAccount" id="inpToAccount" value=""></INPUT>     
                              ...
                              <div>
                                 <span class="dojoTabcurrentfirst">
                                    <div>
                                       ...
                                       <%= Resource.getProperty("finance.budgetprojectfundsavailable.title",lang).replace("'", "\\\'") %>
                                       </a></span>
                                    </div>
                                 </span>
                              </div>
                                       ...
                                       <TABLE  align="center" width="100%">
                                          ...
                                          <tr>
                                             ...
                                             <td class="TextBox_ContentCell">
                                                <select id="inpBudgetYearId" class="ComboKey Combo_TwoCells_width" onchange="getBudgetAccounts(this.value)">
                                                   <%   if(budYrs.length() > 0){
                                          ...
                                          <tr>
                                             <td class="TitleCell"><span class="LabelText" id="inpBudgetItemFromLabel"> <b> <%=Resource.getProperty("finance.budgetitemfrom", lang).replace("'", "\\\'")%>
                                                </b>
                                                </span>
                                             </td>
                                             <td class="TextBox_ContentCell">
                                                <select id="inpBudgetItemFrom" class="ComboKey Combo_TwoCells_width">
                                                <%-- <%   if(budAccts.length() > 0){
                                                       for (int i = 0; i < budAccts.length(); i++) {
                                                           JSONObject objects = budAccts.getJSONObject(i);%>
                                                   <option value='<%=  objects.get("id")%>' ><span><%= objects.get("recordIdentifier")%></span></option>
                                                   <%}}%> --%>
                                                </select>
                                             </td>
                                          </tr>
                                          <tr>
                                             <td class="TitleCell"><span class="LabelText" id="inpBudgetItemToLabel"> <b> <%=Resource.getProperty("finance.budgetitemto", lang).replace("'", "\\\'")%>
                                                </b>
                                                </span>
                                             </td>
                                             <td class="TextBox_ContentCell">
                                                <select id="inpBudgetItemTo" class="ComboKey Combo_TwoCells_width">
                                                <%-- <%   if(budAccts.length() > 0){
                                                       for (int i = 0; i < budAccts.length(); i++) {
                                                           JSONObject objects = budAccts.getJSONObject(i);%>
                                                   <option value='<%=  objects.get("id")%>' ><span><%= objects.get("recordIdentifier")%></span></option>
                                          ...
                                          <tr>
                                             <td class="TitleCell"><span class="LabelText" id="inpViewLabel"> <b> <%=Resource.getProperty("finance.budgetdetailreport.field.view", lang).replace("'", "\\\'")%>
                                                </b>
                                                </span>
                                             </td>
                                             <td class="TextBox_ContentCell">
                                                <select id="inpView" name="inpView" class="ComboKey Combo_TwoCells_width">
                                                <option value='mot' ><span><%=Resource.getProperty("finance.budgetdetailreport.field.mot", lang).replace("'", "\\\'")%></span></option>
                                                <option value='mof' ><span><%=Resource.getProperty("finance.budgetdetailreport.field.mof", lang).replace("'", "\\\'")%></span></option>
                                                </select>
                                             </td>
                                          </tr>
                                          <TR  align = "center">
...
function reloadWindow() {
    submitCommandForm('DEFAULT', false, null, 'BudgetProjectFundsAvailable', '_self', null, false);
    return false;
}
...
function generateReport() {
    ...
    document.getElementById("inpFromAccount").value = document.getElementById("inpBudgetItemFrom").value;
    document.getElementById("inpToAccount").value = document.getElementById("inpBudgetItemTo").value; 
    document.getElementById("inpAction").value = "Submit";    
    openServletNewWindow('PRINT_PDF', true, null, 'BudgetProjectFundsAvailable', null, false, '700', '1000', true);
    return false;
}
...
(function( $ ) {
    $.widget( "ui.combobox", {
        _create: function() {
            ...
            var input = this.input = $( "<input  placeholder=\"<%= Resource.getProperty("scm.select", lang) %>\">" )
                ...
                .autocomplete({
                        ...
                        change: function( event, ui ) {                            
                            ...
                            if($(select).attr("id")=='inpBudgetItemFrom')
                            {       
                                // Selected an item, nothing to do
                                if ( ui.item ) {
                                    return;
                                ...
                                var typedValue = $('#inpBudgetItemFrom').next().val();
                                // Search for a match (case-insensitive)
                                var value = typedValue,
                                    valueLowerCase = value.toLowerCase(),
                                    valid = false;
                                
                                $("#inpBudgetItemFrom > option").each(function() {
                                    if ( this.text.toLowerCase() === valueLowerCase ) {
                                        valid = true;
                                        allowPrint = true;
                                        return false;
                                ...
                                $('#inpBudgetItemFrom').next().val( "" );
                            }
                            if($(select).attr("id")=='inpBudgetItemTo')
                            {       
                                // Selected an item, nothing to do
                                if ( ui.item ) {
                                    return;
                                ...
                                var typedValue = $('#inpBudgetItemTo').next().val();
                                // Search for a match (case-insensitive)
                                var value = typedValue,
                                    valueLowerCase = value.toLowerCase(),
                                    valid = false;
                                
                                $("#inpBudgetItemTo > option").each(function() {
                                    if ( this.text.toLowerCase() === valueLowerCase ) {
                                        valid = true;
                                        allowPrint = true;
                                        return false;
                         ...
                         search: function( event, ui ) { 
                            ...
                            if($(select).attr("id")=='inpBudgetItemFrom'  )
                            {           
                                if($("#inpBudgetItemFrom").next().val()=="")
                                    $("#inpBudgetItemFrom").val("");
                            }
                            if($(select).attr("id")=='inpBudgetItemTo'  )
                            {           
                                if($("#inpBudgetItemTo").next().val()=="")
                                    $("#inpBudgetItemTo").val("");
                            }
                        }                     
                });
            ...
            else if($(select).attr("id")=='inpBudgetItemFrom')                
                input.addClass( "ui-widget autocomplete-widget-content ui-corner-left" );
            else if($(select).attr("id")=='inpBudgetItemTo')                
                input.addClass( "ui-widget autocomplete-widget-content ui-corner-left" );
            else
...
getBudgetAccounts($("#inpBudgetYearId").val());
function getBudgetAccounts(yearId){
    document.getElementById("inpBudgetItemFrom").value = "0";
    document.getElementById("inpBudgetItemTo").value = "0";
        $("#inpBudgetItemFrom").select2(selectBoxAjaxPaging({
            url : function() {
                return '<%=request.getContextPath()%>/BudgetProjectFundsAvailableAjax?action=getBudgetAccounts&yearId='+yearId
           },
           size : "small", placeholder: true
       }));
        $("#inpBudgetItemTo").select2(selectBoxAjaxPaging({
            url : function() {
                return '<%=request.getContextPath()%>/BudgetProjectFundsAvailableAjax?action=getBudgetAccounts&yearId='+yearId
           },
           size : "small", placeholder: true
