package sa.elm.ob.finance.ad_reports.budgetprojectsummary;
 ...
 * 
 */
public class BudgetProjectSummary extends HttpSecureAppServlet {
  private static final long serialVersionUID = 1L;
  private String jspPage = "../web/sa.elm.ob.finance/jsp/budgetprojectsummary/BudgetProjectSummary.jsp";
  ...
  public void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
      ...
      if (action.equals("")) {
        ...
        parameters.put("inpview",
            inpview.equals("mof")
                ? " and org.ad_org_id  in (select a.ad_org_id from ehcm_hrorg_classfication a join ehcm_org_classfication b on b.ehcm_org_classfication_id=a.ehcm_org_classfication_id where b.classification='FIN' and enabled='Y' and a.isactive='Y'"
                    ...
                    + "' and c.c_salesregion_id in (select hq_budgetcontrolunit from efin_budget_ctrl_param where ad_client_id='"
                    + vars.getClient() + "')"
                : " and org.ad_org_id  in (select a.ad_org_id from ehcm_hrorg_classfication a join ehcm_org_classfication b on b.ehcm_org_classfication_id=a.ehcm_org_classfication_id where b.classification='FIN' and enabled='Y' and a.isactive='Y'"
                    + "and a.ad_client_id='" + vars.getClient() + "') and org.ad_client_id='"
                    + vars.getClient()
                    + "' and c.c_salesregion_id in (select budgetcontrol_costcenter from efin_budget_ctrl_param where ad_client_id='"
                    + vars.getClient() + "')");
        ...
        strReportName = "@basedesign@/sa/elm/ob/finance/ad_reports/budgetprojectsummary/BudgetProjectSummary.jrxml";
