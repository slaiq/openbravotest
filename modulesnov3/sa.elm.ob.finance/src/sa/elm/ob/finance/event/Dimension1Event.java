...
public class Dimension1Event extends EntityPersistenceEventObserver {

  private static Entity[] entities = {
      ModelProvider.getInstance().getEntity(UserDimension1.ENTITY_NAME) };
  ...
  public void onUpdate(@Observes EntityUpdateEvent event) {
    ...
    final Property value = entities[0].getProperty(UserDimension1.PROPERTY_SEARCHKEY);
  ...
  public void onDelete(@Observes EntityDeleteEvent event) {
    ...
    try {
      ...
      UserDimension1 user1 = (UserDimension1) event.getTargetInstance();

      final OBCriteria<AcctSchemaElement> acctschemas = OBDal.getInstance()
          .createCriteria(AcctSchemaElement.class);

      acctschemas.add(Restrictions.eq(AcctSchemaElement.PROPERTY_EFINUSER1, user1));
  ...
  private void verifyAccountDimension(EntityPersistenceEvent event) {
    try {
      ...
      ac.add(Restrictions.eq(AccountingCombination.PROPERTY_STDIMENSION, user1));
