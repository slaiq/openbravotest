...
public class SalesRegionEvent extends EntityPersistenceEventObserver {
  ...
  public void onUpdate(@Observes EntityUpdateEvent event) {
    ...
    final Property value = entities[0].getProperty(SalesRegion.PROPERTY_SEARCHKEY);
  ...
  private void verifyAccountDimension(EntityPersistenceEvent event) {
    try {
      ...
      SalesRegion salergn = (SalesRegion) event.getTargetInstance();

      final OBCriteria<AccountingCombination> ac = OBDal.getInstance()
          .createCriteria(AccountingCombination.class);
      ac.add(Restrictions.eq(AccountingCombination.PROPERTY_SALESREGION, salergn));
