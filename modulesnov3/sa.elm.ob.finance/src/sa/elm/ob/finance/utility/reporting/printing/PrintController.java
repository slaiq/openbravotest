  ...
  void post(HttpServletRequest request, HttpServletResponse response, VariablesSecureApp vars,
    ...
    try {
      ...
      if (vars.commandIn("PRINT")) {
        ...
        Invoice invoice = OBDal.getInstance().get(Invoice.class, documentIds[0]);
        DMSIntegrationLog dmslog = DMSUtility.createDMSIntegrationLog(invoice, DMSConstants.DMS_GET,
            null);
        GRPDmsInterface dmsGRP = new GRPDmsImplementation();
        GetAttachmentGRPResponse dmsResponse = null;

        String filename = "Invoice" + "_" + (invoice.getDocumentNo()) + "." + "pdf";
        String profileURI = GetServiceAccount.getProperty("ProfileURI");
        String fileData = "", attachmentURI = "";
        ...
        if (!isPrepaymentApp && !invoice.getDocumentStatus().equals("DR")
            && invoice.getEutAttachPath() != null) {
          attachmentURI = invoice.getEutAttachPath();
          dmsResponse = dmsGRP.getReportFromDMS(attachmentURI);
          if (dmsResponse != null) {
            // response.setDocumentName(outputFile.getName());
            if (!dmsResponse.isError()) {
              ...
              // Download report.
              fileData = dmsResponse.getBase64Str();
              ...
              /*
               * PRINT option will print directly to the UI for a single report. For multiple
               * reports the documents will each be saved individually and the concatenated in the
               * same manner as the saved reports. After concatenating the reports they will be
               * deleted.
               */
              Report report = null;
          ...
          /*
           * PRINT option will print directly to the UI for a single report. For multiple reports
           * the documents will each be saved individually and the concatenated in the same manner
           * as the saved reports. After concatenating the reports they will be deleted.
           */
          Report report = null;
          JasperPrint jasperPrint = null;
          Collection<JasperPrint> jrPrintReports = new ArrayList<JasperPrint>();
