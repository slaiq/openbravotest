...
package sa.elm.ob.finance.ad_actionbutton.NonMOFReconcile;
...
public class TransactionsDao {
  ...
  public static FieldProvider[] getTransactionsFiltered(FIN_FinancialAccount account,
    ...
    try {
      ...
      if (strValidBank.equals("Y")) {
        whereClause.append(" and (pym.efinPayReceiveDate is not null ) ");
        whereClause
            .append(" and (pym.efinPayReceivedby is not null and pym.efinPayReceivedby <>'' ) ");
      }
      parameters.add(account.getId());
      if (hideAfterDate) {
        whereClause.append(" and ft.transactionDate < ?");
      ...
      if (sortColumn.isEmpty())
        whereClause.append(" order by pym.documentNo asc ");
      else if (sortColumn.equals("Date"))
        whereClause.append(" order by ft.transactionDate ");
      else if (sortColumn.equals("ReceivedDate")) {
        whereClause.append(" order by pym.efinPayReceiveDate ");
      ...
      for (int i = 0; i < data.length; i++) {
        ...
        // Truncate payment Details

        if (FIN_Transactions[i].getFinPayment().getEfinPayReceiveDate() != null
            && !FIN_Transactions[i].getFinPayment().getEfinPayReceiveDate().equals("")) {
          String paymentReceivedDate = dateFormater
              .format(FIN_Transactions[i].getFinPayment().getEfinPayReceiveDate());
          paymentReceivedDate = dateYearFormat.format(dteFormat.parse(paymentReceivedDate));
          paymentReceivedDate = UtilityDAO.convertTohijriDate(paymentReceivedDate);// Hijri Date
          FieldProviderFactory.setField(data[i], "PaymentReceivedDate", paymentReceivedDate);
        } else {
          FieldProviderFactory.setField(data[i], "PaymentReceivedDate", "");
        }
        if (FIN_Transactions[i].getFinPayment().getEfinPayReceivedby() != null
            && !FIN_Transactions[i].getFinPayment().getEfinPayReceivedby().equals("")) {
          FieldProviderFactory.setField(data[i], "PaymentReceivedBy",
              FIN_Transactions[i].getFinPayment().getEfinPayReceivedby());
        } else {
          FieldProviderFactory.setField(data[i], "PaymentReceivedBy", "");
        }
