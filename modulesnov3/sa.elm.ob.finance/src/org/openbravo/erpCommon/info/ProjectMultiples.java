...
public class ProjectMultiples extends HttpSecureAppServlet {
	private static final long serialVersionUID = 1L;

	private static final String[] colNames = { "Value", "Name", "Project_Category", "C_Project_ID", "RowKey" };
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws IOException, ServletException {
		...
		if(vars.commandIn("DEFAULT")) {
			...
			// "ProjectMultiples.name");
			// if (!strNameValue.equals(""))
			// vars.setSessionValue("ProjectMultiples.name", strNameValue + "%");
			String strKeyValue = vars.getGlobalVariable("inpKey", "ProjectMultiples.key", "");
			String strNameValue = vars.getGlobalVariable("inpName", "ProjectMultiples.name", "");
			...
			if(vars.getStringParameter("clear").equals("true")) {
				vars.removeSessionValue("ProjectMultiples.key");
				vars.removeSessionValue("ProjectMultiples.name");
				vars.removeSessionValue("ProjectMultiples.adorgid");
			}
			String strKey = vars.getGlobalVariable("inpKey", "ProjectMultiples.key", "");
			String strName = vars.getGlobalVariable("inpName", "ProjectMultiples.name", "");
			String strOrg = vars.getGlobalVariable("inpAD_Org_ID", "ProjectMultiples.adorgid", "");
	...
	private void PrintPage(HttpServletResponse response, VariablesSecureApp vars, String strKeyValue, String strNameValue) throws IOException, ServletException {
		...
		XmlDocument xmlDocument = xmlEngine.readXmlTemplate("org/openbravo/erpCommon/info/ProjectMultiples").createXmlDocument();
		...
		xmlDocument.setParameter("alertMsg", "ALERT_MSG=\"" + Utility.messageBD(this, "NoProjectIsSelected", vars.getLanguage()) + "\";");
	...
	private SQLReturnObject[] getHeaders(VariablesSecureApp vars) {
		...
		for (int i = 0; i < colNames.length; i++) {
			...
			dataAux.setData("isvisible", (colNames[i].equals("C_Project_ID") || colNames[i].equalsIgnoreCase("RowKey") ? "false" : "true"));
		...
		if(headers != null) {
			try {
				...
				if(strNewFilter.equals("1") || strNewFilter.equals("")) { // New
					...
					strNumRows = ProjectMultiplesData.countRows(this, rownum, strKey, strName, Utility.getContext(this, vars, "#User_Client", "ProjectMultiples"), Utility.getSelectorOrgs(this, vars, strOrg), vars.getRole(), pgLimit, oraLimit1, oraLimit2);
				...
				if(this.myPool.getRDBMS().equalsIgnoreCase("ORACLE")) {
					...
					data = ProjectMultiplesData.select(this, "ROWNUM", strKey, strName, Utility.getContext(this, vars, "#User_Client", "ProjectMultiples"), Utility.getSelectorOrgs(this, vars, strOrg), vars.getRole(), strOrderBy, "", oraLimit1, oraLimit2);
				}
				else {
					String pgLimit = pageSize + " OFFSET " + offset;
					data = ProjectMultiplesData.select(this, "1", strKey, strName, Utility.getContext(this, vars, "#User_Client", "ProjectMultiples"), Utility.getSelectorOrgs(this, vars, strOrg), vars.getRole(), strOrderBy, pgLimit, "", "");
				}
			}
