...
class AuditTrailDeletedRecords {
  ...
  static FieldProvider[] getDeletedRecords(ConnectionProvider conn, VariablesSecureApp vars,
    ...
    try {
      ...
      sql.append("  AND AD_COLUMN_ID = '").append(ModelProvider.getInstance()
          .getEntityByTableName(tableName).getIdProperties().get(0).getColumnId()).append("'\n");

      // Add security filter
      sql.append(" AND AD_CLIENT_ID IN (")
          .append(Utility.getContext(conn, vars, "#User_Client", tab.getWindow().getId()))
          .append(")\n");
      sql.append(" AND AD_ORG_ID IN (").append(Utility.getContext(conn, vars, "#AccessibleOrgTree",
          tab.getWindow().getId(), Integer.parseInt(tab.getTable().getDataAccessLevel())))
          .append(")\n");
