(function($, undefined) {
	...
	Time.prototype.GT = function(time) {
		return (this.hours > time.hours
				|| (this.hours == time.hours && this.minutes > time.minutes) || (this.hours == time.hours
				&& this.minutes == time.minutes && this.seconds > time.seconds)) ? true
				: false
	};
	...
	Time.prototype.equals = function(time) {
		return (this.hours == time.hours && this.minutes == time.minutes && this.seconds == time.seconds) ? true
				: false
	};
	Time._isValidFormat = function(format) {
		var match = (format
				.match(/(^((TT|tt|T|t)(\s)?)?h{1,2}((:|\.)m{1,2}((:|\.)s{1,2})?)?$)|(^h{1,2}((:|\.)m{1,2}((:|\.)s{1,2})?)?((\s)?(TT|tt|T|t))?$)|(^m{1,2}((:|\.)s{1,2})?$)|(^s{1,2}$)/g) !== null);
		var matchSep1 = (format.match(/:/) === null);
		var matchSep2 = (format.match(/\./) === null);
		var matchSep = (matchSep1 && matchSep2) || (matchSep1 != matchSep2);
		return (match && matchSep)
	};
	Time._isValidTime = function(time, format) {
		var is12HourFormat = Time._is12HourFormat(format);
		var numTimeFormat = format.replace(
				/hh/g,
				is12HourFormat ? "((0[0-9])|(1[0-2]))"
						: "(([0-1][0-9])|(2[0-4]))").replace(
				/h/g,
				is12HourFormat ? "(([0-9])|(1[0-2]))"
						: "(([0-9])|(1[0-9])|(2[0-4]))").replace(/mm/g,
				"([0-5][0-9])").replace(/m/g, "(([1-9])|([1-5][0-9]))")
				.replace(/ss/g, "([0-5][0-9])").replace(/s/g,
						"(([1-9])|([1-5][0-9]))").replace(/TT/g, "(AM|PM)")
				.replace(/tt/g, "(am|pm)").replace(/T/g, "(A|P)").replace(/t/g,
						"(a|p)");
	...
	Time.prototype.formatTime = function(format) {
		...
		var hours = Time._is12HourFormat(timeFormat) ? (this.hours > 12 ? this.hours % 12
				: (this.hours == 0 ? 12 : this.hours))
				: this.hours;
		formattedTime = timeFormat
				...
				.replace(/m/g, this.minutes).replace(/ss/g,
						((this.seconds < 10) ? "0" : "") + this.seconds)
				.replace(/s/g, this.seconds).replace(/TT/g, ampm.toUpperCase())
				.replace(/tt/g, ampm.toLowerCase()).replace(/T/g,
						ampm.charAt(0).toUpperCase()).replace(/t/g,
						ampm.charAt(0).toLowerCase());
		return formattedTime
	};
	Time.prototype.set = function(hours, minutes, seconds) {
		var hours = (hours === undefined || (typeof hours != "number")) ? 0
				: Math.round(hours);
		var minutes = (minutes === undefined || (typeof minutes != "number")) ? 0
				: Math.round(minutes);
		var seconds = (seconds === undefined || (typeof minutes != "number")) ? 0
				: Math.round(seconds);
	...
	Time.prototype.setFromTime = function(time) {
		var hours = (time.hours === undefined || (typeof time.hours != "number")) ? 0
				: Math.round(time.hours);
		var minutes = (time.minutes === undefined || (typeof time.minutes != "number")) ? 0
				: Math.round(time.minutes);
		var seconds = (time.seconds === undefined || (typeof time.seconds != "number")) ? 0
	...
	Time.prototype.setFromString = function(time, format) {
		...
		var ampm = is12HourFormat ? (timeString.match(/a/ig) ? "AM" : "PM")
				: "";
		var timeString = timeString.replace(/(am|a|pm|p)/ig, "").replace(/\s/g,
				"");
		var separator = timeString.match(/:/g) !== null ? ":" : (timeString
				.match(/\./g) !== null ? "." : " ");
		var hours = 0;
		var minutes = 0;
		var seconds = 0;
		...
		if (timeFormat.search(/h/g) != -1) {
			...
			hours = (ampm == "PM" && hours != 12) ? hours + 12
	...
	function Dateplustimepicker() {
		...
		this._defaults = {
			...
			onDateTimeChangeStop : null,
			onTimeChange : null,
			onTimeChangeStop : null,
			secondGrid : 0,
			showTime : true,
			step : {
				hours : 0,
			...
			.extend(
					Dateplustimepicker.prototype,
					{
						...
						_attachDateplustimepicker : function(target, settings) {
							...
							inst.timepicker.settings = $.extend({}, settings
									|| {}, inlineSettings || {});
							if (nodeName == "input") {
								...
								if (inline) {
									this
											._inlineDateplustimepicker(target,
													inst)
								}
							}
						},
						...
						_attachments : function(input, inst) {
							var $input = input;
							var $altTimeField = $($.dateplustimepicker._get(
									inst.timepicker, "altTimeField"));
							var timeOnly = $.dateplustimepicker._get(
									inst.timepicker, "timeOnly");
							if (timeOnly) {
								$input.unbind("keydown").unbind("keypress")
										.keypress(this._doKeyPressTime).unbind(
												"keyup").keyup(
												this._doKeyUpTime)
							} else {
								if ($altTimeField.length > 0) {
									$input.unbind("keydown").keydown(
											$.datepicker._doKeyDown).unbind(
											"keypress").keypress(
											$.datepicker._doKeyPress).unbind(
											"keyup").keyup(
											$.datepicker._doKeyUp);
									$altTimeField.unbind("keydown").keydown(
											$.datepicker._doKeyDown).unbind(
											"keypress").keypress(
											this._doKeyPressTime).unbind(
											"keyup").keyup(this._doKeyUpTime)
								} else {
									$input.unbind("keydown").keydown(
											$.datepicker._doKeyDown).unbind(
											"keypress").keypress(
											this._doKeyPress).unbind("keyup")
											.keyup($.datepicker._doKeyUp)
											.keyup(this._doKeyUp)
								}
							}
							if ($altTimeField.length > 0) {
								$.data($altTimeField[0], "datepicker", inst);
								$altTimeField
										.addClass($.datepicker.markerClassName);
								var _onFocus = function() {
									$.datepicker._showDatepicker($input[0]);
									$altTimeField.unbind("focus");
									setTimeout(function() {
										$altTimeField.focus()
									}, 200)
								};
								...
								$altTimeField.blur(function() {
									$altTimeField.unbind("focus").focus(
											_onFocus)
								})
							}
						},
						_autoSize : function(inst) {
							if ($.datepicker._get(inst, "autoSize")
									&& !inst.inline) {
								var time = new Time(24, 59, 59);
								var timeFormat = $.dateplustimepicker._get(
										inst.timepicker, "timeFormat");
								var maxTimeLength = time.formatTime(timeFormat).length;
								var $altTimeField = $($.dateplustimepicker
										._get(inst.timepicker, "altTimeField"));
								if ($altTimeField.length > 0) {
									$altTimeField.attr("size", maxTimeLength)
								} else {
									var inputLength = inst.input.attr("size");
									inst.input.attr("size", inputLength + 1
											+ maxTimeLength)
								}
							}
						},
						_inlineDateplustimepicker : function(target, inst) {
							...
							var defTime = new Time($.dateplustimepicker._get(
									inst.timepicker, "defaultTime"));
							inst.timepicker.time = defTime;
							inst.timepicker.time = $.dateplustimepicker
									._constraintTime(inst);
							$.datepicker._updateDatepicker(inst);
							$.dateplustimepicker.updateDateTime(inst)
						},
						_get : function(timepicker, name) {
							return timepicker.settings[name] !== undefined ? timepicker.settings[name]
									: this._defaults[name]
						},
						_optionDateTimePicker : function(target, name, value) {
							...
							if (arguments.length == 2
								...
								return (name == "defaults" ? $.extend({},
										...
										$.dateplustimepicker._defaults)
										: (inst ? (name == "all" ? $.extend({},
												inst.settings)
												: ($.dateplustimepicker._get(
														inst, name) || $.datepicker
														._get(inst, name)))
												: null))
							}
							var settings = name || {};
							...
							if (settings.altTimeField !== undefined) {
								var oldAltTimeField = $($.dateplustimepicker
										._get(inst.timepicker, "altTimeField"));
								oldAltTimeField.val("");
								oldAltTimeField.unbind("keydown").unbind(
										"keypress").unbind("keyup").unbind(
										"focus").unbind("blur")
							}
							if (inst) {
								...
								var date = $.dateplustimepicker
										._getDateTimeDateTimePicker(target,
												true);
								...
								if (!inline) {
									...
									$.dateplustimepicker._attachments(
											$(target), inst);
									$.datepicker._autoSize(inst);
									$.dateplustimepicker._autoSize(inst)
								}
								$.dateplustimepicker
										._setDateTimeDateTimePicker(target,
												date);
								$.dateplustimepicker
										._setDateTimeFromField(inst);
								$.datepicker._updateDatepicker(inst)
							}
						},
						injectTimePicker : function(inst) {
							...
							if ($(".ui-dateplustimepicker-div", $dp).length === 0) {
								var html = $.dateplustimepicker
										._generateHtml(inst);
								$tp = $(html);
								var showButtonPanel = $.datepicker._get(inst,
										"showButtonPanel");
								if (showButtonPanel) {
									$dp.find(".ui-datepicker-buttonpane")
											.before($tp)
								} else {
									$dp.append($tp)
								}
								...
								var timeOnly = $.dateplustimepicker._get(
										inst.timepicker, "timeOnly");
								if (timeOnly === true) {
									var timeOnlyTitle = $.dateplustimepicker
						...
						_generateHtml : function(inst) {
							...
							var showTime = $.dateplustimepicker._get(
									inst.timepicker, "showTime");
							var timeText = $.dateplustimepicker._get(
									inst.timepicker, "timeText");
							var format = $.dateplustimepicker._get(
									inst.timepicker, "timeFormat");
							var time = inst.timepicker.time.formatTime(format);
							var showHour = format.match(/h/) !== null;
							var hourText = $.dateplustimepicker._get(
									inst.timepicker, "hourText");
							var hourGrid = $.dateplustimepicker._get(
									inst.timepicker, "hourGrid");
							var showMinute = format.match(/m/) !== null;
							var minuteText = $.dateplustimepicker._get(
									inst.timepicker, "minuteText");
							var minuteGrid = $.dateplustimepicker._get(
									inst.timepicker, "minuteGrid");
							var showSecond = format.match(/s/) !== null;
							var secondText = $.dateplustimepicker._get(
									inst.timepicker, "secondText");
							var secondGrid = $.dateplustimepicker._get(
									inst.timepicker, "secondGrid");
							var html = '<div class="ui-dateplustimepicker-div">';
							html += '<div class="ui-dateplustimepicker-time"'
									+ (showTime ? "" : noDisplay) + ">";
							html += '<div class="ui-dateplustimepicker-time-label ui-dateplustimepicker-label">'
									+ timeText + "</div>";
							html += '<div class="ui-dateplustimepicker-time-value ui-dateplustimepicker-value">'
									+ time + "</div>";
							html += "</div>";
							html += '<div class="ui-dateplustimepicker-hour"'
									+ (showHour ? "" : noDisplay) + ">";
							html += '<div class="ui-dateplustimepicker-hour-label ui-dateplustimepicker-label">'
									+ hourText + "</div>";
							...
							html += '<div class = "ui-dateplustimepicker-hour-grid ui-dateplustimepicker-grid"'
									+ (hourGrid > 0 ? "" : noDisplay)
									+ "></div>";
							html += "</div>";
							html += "</div>";
							html += '<div class="ui-dateplustimepicker-minute"'
									+ (showMinute ? "" : noDisplay) + ">";
							html += '<div class="ui-dateplustimepicker-minute-label ui-dateplustimepicker-label">'
									+ minuteText + "</div>";
							...
							html += '<div class = "ui-dateplustimepicker-minute-grid ui-dateplustimepicker-grid"'
									+ (minuteGrid > 0 ? "" : noDisplay)
									+ "></div>";
							html += "</div>";
							html += "</div>";
							html += '<div class="ui-dateplustimepicker-second"'
									+ (showSecond ? "" : noDisplay) + ">";
							html += '<div class="ui-dateplustimepicker-second-label ui-dateplustimepicker-label">'
									+ secondText + "</div>";
							...
							html += '<div class = "ui-dateplustimepicker-second-grid ui-dateplustimepicker-grid"'
									+ (secondGrid > 0 ? "" : noDisplay)
									+ "></div>";
							html += "</div>";
							html += "</div>";
							html += "</div>";
						...
						_addSliders : function(inst) {
							...
							var showSecond = format.match(/s/) !== null;
							if (showHour) {
								$(".ui-dateplustimepicker-hour-slider", $dp)
										.slider(
												{
							...
							if (showMinute) {
								$(".ui-dateplustimepicker-minute-slider", $dp)
										.slider(
												{
													orientation : "horizontal",
							...
							if (showSecond) {
								$(".ui-dateplustimepicker-second-slider", $dp)
										.slider(
												{
													orientation : "horizontal",
						...
						_addGrids : function(inst) {
							...
							var showMinute = format.match(/m/) !== null;
							var showSecond = format.match(/s/) !== null;
							var minTime = new Time($.dateplustimepicker._get(
									inst.timepicker, "minTime"));
							var maxTime = new Time($.dateplustimepicker._get(
									inst.timepicker, "maxTime"));
							var hourMin = minTime.hours;
							var hourMax = maxTime.hours;
							var ampm = Time
									._is12HourFormat($.dateplustimepicker._get(
											inst.timepicker, "timeFormat"));
							var time = new Time();
							if (showHour && hourGrid > 0) {
								...
								for ( var h = 0; h < 24; h += hourGrid) {
									time.hours = h;
									var hour = ampm ? time.formatTime("hht")
											: time.formatTime("hh");
									html += '<td width="'
											+ tdWidth
											+ '%"><span'
											+ ((h < hourMin || h > hourMax) ? ' style="display:none;">'
													: ">") + hour
											+ "</span></td>"
								}
								html += "</tr></table>";
								$(
										".ui-dateplustimepicker-hour .ui-dateplustimepicker-hour-grid",
										$dp).append(html);
								$(".ui-dateplustimepicker-hour span", $dp)
										.each(
												function(index) {
													$(this)
															.click(
							...
							if (showMinute && minuteGrid > 0) {
								...
								html += "</tr></table>";
								$(
										".ui-dateplustimepicker-minute .ui-dateplustimepicker-minute-grid",
										$dp).append(html);
								$(".ui-dateplustimepicker-minute span", $dp)
										.each(
												function(index) {
													$(this)
															.click(
							...
							if (showSecond && secondGrid > 0) {
								...
								html += "</tr></table>";
								$(
										".ui-dateplustimepicker-second .ui-dateplustimepicker-second-grid",
										$dp).append(html);
								$(".ui-dateplustimepicker-second span", $dp)
										.each(
												function(index) {
													$(this)
															.click(
						...
						_updateTimeDisplay : function(inst) {
							...
							var time = inst.timepicker.time.formatTime(format);
							var timeDisplay = $(
									".ui-dateplustimepicker-time-value",
									inst.dpDiv);
							timeDisplay.text(time)
						},
						_updateSliders : function(inst, event) {
							var hourSlider = $(
									".ui-dateplustimepicker-hour-slider",
									inst.dpDiv);
							var minuteSlider = $(
									".ui-dateplustimepicker-minute-slider",
									inst.dpDiv);
							var secondSlider = $(
									".ui-dateplustimepicker-second-slider",
									inst.dpDiv);
							hourSlider.slider("option", "value",
									inst.timepicker.time.hours);
							minuteSlider.slider("option", "value",
									inst.timepicker.time.minutes);
							secondSlider.slider("option", "value",
									inst.timepicker.time.seconds);
						...
						_constraintTime : function(inst, slider) {
							...
							var step = new Time($.dateplustimepicker._get(
									inst.timepicker, "step"));
							step.hours = hasHours ? step.hours : 0;
							step.minutes = hasMinutes ? step.minutes : 0;
							step.seconds = hasSeconds ? step.seconds
									: (step.hours != 0 || step.minutes != 0) ? 0
											: 1;
							...
							maxTimeInSeconds = maxTimeInSeconds
									- (maxTimeInSeconds - minTimeInSeconds)
									% stepInSeconds;
							var timeInSeconds = inst.timepicker.time.getTime();
							var steps = Math
									.round((timeInSeconds - minTimeInSeconds)
											/ stepInSeconds);
							steps = steps < 0 ? 0 : steps;
							timeInSeconds = minTimeInSeconds + steps
									* stepInSeconds;
							timeInSeconds = timeInSeconds > maxTimeInSeconds ? maxTimeInSeconds
									: timeInSeconds;
						...
						_notifyTimeChange : function(inst, fromPicker) {
							...
							if (onTimeChange) {
								onTimeChange.apply((inst.input ? inst.input[0]
										: null), [ inst.timepicker.time, inst,
										fromPicker ])
							}
						},
						_notifyDateTimeChange : function(inst, fromPicker) {
							var onDateTimeChange = this._get(inst,
									"onDateTimeChange");
							var date = this._getDateTime(inst);
							if (onDateTimeChange) {
								onDateTimeChange.apply(
										(inst.input ? inst.input[0] : null), [
												date, inst, fromPicker ])
							}
						},
						_notifyTimeChangeStop : function(inst, fromPicker) {
							var onTimeChangeStop = this._get(inst,
									"onTimeChangeStop");
							if (onTimeChangeStop) {
								onTimeChangeStop.apply(
										(inst.input ? inst.input[0] : null), [
						...
						_notifyDateTimeChangeStop : function(inst, fromPicker) {
							var onDateTimeChangeStop = this._get(inst,
									"onDateTimeChangeStop");
							var date = this._getDateTime(inst);
							if (onDateTimeChangeStop) {
								onDateTimeChangeStop.apply(
										(inst.input ? inst.input[0] : null), [
												date, inst, fromPicker ])
							}
						},
						updateDateTime : function(inst) {
							...
							var timeOnly = $.dateplustimepicker._get(
									inst.timepicker, "timeOnly");
							var alwaysSetTime = $.dateplustimepicker._get(
									inst.timepicker, "alwaysSetTime");
						...
						_getDateTimeDateTimePicker : function(target, noDefault) {
							var inst = $.datepicker._getInst(target);
							if (inst && !inst.inline) {
								$.dateplustimepicker._setDateTimeFromField(
										inst, noDefault)
							}
							return (inst ? $.dateplustimepicker
									._getDateTime(inst) : null)
						},
						_getDateTime : function(inst) {
							var date = (!inst.currentYear
						...
						_setDateTimeDateTimePicker : function(target, date,
								noChange) {
							if (date === null) {
								return
							}
							...
							if (inst) {
								if (typeof date == "object" && date !== null) {
									$.datepicker._setDate(inst, new Date(date
											.getTime()), noChange)
								} else {
									$.datepicker._setDate(inst, date, noChange)
								}
								$.dateplustimepicker._setTimeDateTime(inst,
										date);
								$.dateplustimepicker.updateDateTime(inst);
								$.datepicker._updateDatepicker(inst)
							}
						...
						_setTimeDateTime : function(inst, date, noChange) {
							if (typeof date == "object" && date !== null) {
								$.dateplustimepicker._setTimeFromDate(inst,
										date)
							} else {
								if (typeof date == "string") {
									$.dateplustimepicker
											._setTimeFromDateTimeString(inst,
													date)
								}
							}
							inst.timepicker.time = $.dateplustimepicker
						...
						_setTimeFromDateTimeString : function(inst,
								datetimeString, noDefault) {
							var timeFormat = $.dateplustimepicker._get(
									inst.timepicker, "timeFormat");
							var tokens = datetimeString.split(" ");
							var timeString = tokens.splice(1).join(" ");
							$.dateplustimepicker._setTimeFromString(inst,
									timeString, noDefault)
						},
						_setTimeFromString : function(inst, timeString,
								noDefault) {
							var timeFormat = $.dateplustimepicker._get(
									inst.timepicker, "timeFormat");
							try {
								inst.timepicker.time.setFromString(timeString,
										timeFormat)
							} catch (err) {
								if (noDefault === undefined
										|| noDefault == false) {
									var defTime = new Time($.dateplustimepicker
											._get(inst.timepicker,
													"defaultTime"));
									inst.timepicker.time = defTime
								}
						...
						_setTimeFromDate : function(inst, date) {
							if (inst.timepicker) {
								...
								inst.timepicker.time.minutes = date
										.getMinutes();
								inst.timepicker.time.seconds = date
										.getSeconds()
							}
						},
						...
						_getTimeDateTimePicker : function(target, noDefault) {
							...
							return (inst ? $.dateplustimepicker._getTime(inst)
									: null)
						},
						_getTime : function(inst) {
							var time = (!inst.timepicker.time
									|| (inst.input && inst.input.val() == "") ? null
									: inst.timepicker.time);
							return time
						},
						_setTimeDateTimePicker : function(target, time,
								noChange) {
							if (time === null) {
								return
							}
						...
						_setTime : function(inst, time) {
							if (typeof time == "object") {
								...
								if (typeof time == "string") {
									$.dateplustimepicker._setTimeFromString(
											inst, time, false)
								}
							}
							inst.timepicker.time = $.dateplustimepicker
						...
						_setDateTimeFromField : function(inst, noDefault) {
							...
							if (timeOnly) {
								...
								if ($altTimeField.length > 0) {
									...
									$.dateplustimepicker._setTimeFromString(
											inst, timeString, noDefault)
								} else {
									$.dateplustimepicker
											._setTimeFromDateTimeString(inst,
													inst.input.val(), noDefault)
								}
							}
							inst.timepicker.time = $.dateplustimepicker
									._constraintTime(inst)
						...
						_possibleChars : function(format) {
							...
							var otherChars = format.replace(/[hms]/g, "")
									.replace(/TT/g, "APM")
									.replace(/tt/g, "apm").replace(/T/g, "AP")
									.replace(/t/g, "ap");
							chars += otherChars;
							return chars
						},
						_doKeyPress : function(event) {
							...
							var constrainInput = $.datepicker._get(inst,
									"constrainInput");
							if (constrainInput) {
								...
								var timeFormat = $.dateplustimepicker._get(
										inst, "timeFormat");
								var dateChars = $.datepicker
										._possibleChars(dateFormat);
								var timeChars = $.dateplustimepicker
										._possibleChars(timeFormat);
						...
						_doKeyPressTime : function(event) {
							var inst = $.data(event.target, "datepicker")
						...
						_doKeyUpTime : function(event) {
							...
							var lastVal = ($(event.target) == inst.input) ? inst.timepicker.lastVal
									: inst.timepicker.lastAltFieldVal;
						...
						_doKeyUp : function(event) {
							...
							if (val != lastVal) {
								$.dateplustimepicker
										._setTimeFromDateTimeString(inst, val,
												true);
								inst.timepicker.time = $.dateplustimepicker
										._constraintTime(inst);
								$.datepicker._updateAlternate(inst);
						...
						_updateDateTimePicker : function(inst) {
							$.dateplustimepicker._datepickerUpdateDatePicker
									.apply($.datepicker, [ inst ]);
							if (inst.timepicker) {
						...
						_updateAlternate : function(inst) {
							if (!inst.timepicker) {
								$.dateplustimepicker._datepickerUpdateAlternate
										.apply($.datepicker, [ inst ]);
								return
							}
							var altField = $.datepicker._get(inst, "altField");
							if (altField) {
								var altFormat = $.datepicker._get(inst,
										"altFormat")
										|| $.datepicker
												._get(inst, "dateFormat");
								var date = $.datepicker._getDate(inst);
								var dateStr = $.datepicker.formatDate(
										altFormat, date, $.datepicker
												._getFormatConfig(inst));
								var altTimeFormat = $.dateplustimepicker._get(
										inst.timepicker, "altTimeFormat")
										|| $.dateplustimepicker._get(
												inst.timepicker, "timeFormat");
								var timeStr = inst.timepicker.time
										.formatTime(altTimeFormat);
								var timeOnly = $.dateplustimepicker._get(
										inst.timepicker, "timeOnly");
								datetimeStr = timeOnly ? timeStr : dateStr
										+ " " + timeStr;
								$(altField).each(function() {
									$(this).val(datetimeStr)
								})
							}
						},
						...
						_gotoToday : function(id) {
							...
							if (inst.timepicker) {
								$.dateplustimepicker
										._setDateTimeDateTimePicker(target[0],
												new Date())
							} else {
								$.dateplustimepicker._datepickerGotoToday
										.apply($.datepicker, [ id ])
							}
						},
						_datepickerSetDate : $.datepicker._setDate,
						_setDate : function(inst, date, noChange) {
							$.dateplustimepicker._datepickerSetDate.apply(
									$.datepicker, [ inst, date, noChange ]);
							if (inst.timepicker) {
								setTimeout(function() {
									$.dateplustimepicker.updateDateTime(inst)
								}, 100)
						...
						_selectDate : function(id, dateStr) {
							...
							if (inst.timepicker) {
								...
								inst.inline = true;
								$.dateplustimepicker._datepickerSelectDate
										.apply($.datepicker, [ id, dateStr ]);
								inst.inline = inline;
								$.dateplustimepicker.updateDateTime(inst)
							} else {
								$.dateplustimepicker._datepickerSelectDate
							...
							$.dateplustimepicker._notifyDateTimeChange(inst,
									true);
							$.dateplustimepicker._notifyDateTimeChangeStop(
									inst, true)
						}
					});
	jQuery.fn.dateplustimepicker = function(options) {
		...
		if (typeof (options) == "string") {
			if (options == "getDateTime"
				...
				return $.dateplustimepicker["_" + options + "DateTimePicker"]
						.apply($.dateplustimepicker, [ this[0] ]
								.concat(otherArgs))
			}
			if (options == "setDateTime" || options == "setTime"
					|| options == "option") {
				return this.each(function() {
					$.dateplustimepicker["_" + options + "DateTimePicker"]
							.apply($.dateplustimepicker, [ this ]
									.concat(otherArgs))
				})
			}
			if (options == "dialog") {
				return this.datepicker(options, arguments[1], arguments[2],
	...
	jQuery.fn.timepicker = function(opts) {
		...
		return $(this).dateplustimepicker(opts, arguments[1], arguments[2],
				arguments[3], arguments[4])
	};
	...
	$.dateplustimepicker = new Dateplustimepicker();
