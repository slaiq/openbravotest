...
isc.OBStandardView.addProperties({
    ...
    addChildView: function(childView) {
        if ((childView.isTrlTab && OB.PropertyStore.get('ShowTrl', this.standardWindow.windowId) !== 'Y') || (childView.isAcctTab && OB.PropertyStore.get('ShowAcct', this.standardWindow.windowId) !== 'Y')) {
            return;
        }
    ...
    deleteSelectedRows: function(autoSaveDone) {
        ...
        var formView = this.viewForm,
            gridView = this.viewGrid,
            parentTab;

        if (formView !== undefined) {
            parentTab = formView.view.parentTabSet;
        } else if (gridView !== undefined) {
            parentTab = gridView.view.parentTabSet;
        }
        ...
        if (!this.readOnly && this.isDeleteableTable) {
            ...
            var messageCallBack = function(resp, data, req) {
                    message = data.deleteMessage;
                    msg = message;
                    if (deleteCount === 1) {
                        //OB.I18N.getLabel('OBUIAPP_DeleteConfirmationSingle');
            ...
            OB.RemoteCallManager.call('sa.elm.ob.utility.ad_actionHandler.deleteMessageActionHandler.DeleteMessageActionHandler', {
                tabId: this.tabId,
                numberOfRecords: deleteCount
            }, {}, messageCallBack);
                    ...
                    if (classicMode) {
                        if (propertyObj.type && convertToClassicFormat) {
                            ...
                            if (type.createClassicString) {
                                ...
                                if (type.editorType === 'OBHijriDateItem' && isc.unrealdates.indexOf(value) > -1) {
                                ...
                                if (type.referenceName === 'Secured String') {
                                    contextInfo[properties[i].inpColumn] = btoa(value);
                                }
                            }
                        } else {
                            contextInfo[properties[i].inpColumn] = this.convertContextValue(value, propertyObj.type);
    ...
    getTabMessage: function(forcedTabId) {
        ...
        callback = function(resp, data, req) {
            // PKI integration plugin call
            if (data.isPKI) {
                console.log(data);
                var xaxis, yaxis;

                if (data.documentType === 'EUT_101' && data.level === 'L3') {
                    xaxis = '400';
                    yaxis = '35';
                } else if (data.documentType === 'EUT_101' && data.level === 'L4') {
                    xaxis = '280';
                    yaxis = '35';
                } else if (data.documentType === 'EUT_101' && data.level === 'L5') {
                    xaxis = '130';
                    yaxis = '35';
                } else {
                    xaxis = "20";
                ...
                if (data.documentType === 'EUT_101' && (data.level === 'L1' || data.level === 'L2')) {
        ...
        OB.RemoteCallManager.call('sa.elm.ob.utility.ad_actionHandler.GetTabMessageHandler', {
            tabId: tabId
        }, null, callback, this);
    },
    ...
    prepareGridFields: function(fields) {
        ...
        for (i = 0; i < length; i++) {
            ...
            if (fld.gridProps.displaylength) {
                if (fld.gridProps.displaybasedonwidth != undefined && fld.gridProps.displaybasedonwidth) {
                    fld.gridProps.width = fld.gridProps.gridlength;
                } else {
