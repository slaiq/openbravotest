...
// == OBHijriDateItem ==
// OBHijriDateItem inherits from SmartClient DateItem
// adds autocomplete and formatting based on the Openbravo date pattern
isc.ClassFactory.defineClass('OBHijriDateItem', isc.DateItem);
...
OB.DateItemProperties = {
  ...
  pickerDataChanged: function (picker) {
    ...
    this.updateValue();
    if (this && this.form && this.form.grid && OB.OnChangeRegistry.hasOnChange(this.form.grid.ID, this)) {
  	  OB.OnChangeRegistry.call(this.form.grid.ID, this, this.form.grid.view, this.form.grid.view.theForm, this.form.grid);
  	  this.form.grid.view.theForm.redraw();
    }
...
isc.OBHijriDateItem.addProperties(OB.DateItemProperties, {
  validateOnExit: true,
  showChooserFiscalYearPicker:false,
  startDate: Date.createLogicalDate(1430, 01, 01),
  endDate: Date.createLogicalDate(1448, 01, 01),
...
/*  getInnerHTML : function () {
      return "<input type='text' style='width:100%;height:100%' ID='" + 
                        this.getID() + "_ckEditor' handlenativeevents='true' class='OBFormFieldInputRequired' style='WIDTH:163px;HEIGHT:10px;'></input>";
  },
  ...
  draw : function () {
      ...
      OBHijriDateItem.replace(this.getID+"_ckEditor");
      $("# "+this.getID() + "_ckEditor").calendarsPicker(); 
  ...
  validateOBHijriDateItem: function (value) {
  ...
  validators: [{
    ...
    condition: function (item, validator, value) {
      return item.validateOBHijriDateItem(value);
    }
  }]
});

OB.I18N.getLabel('OBUIAPP_InvalidValue', null, isc.OBHijriDateItem, 'invalidValueLabel');
OB.I18N.getLabel('OBUISC_Validator.requiredField', null, isc.OBHijriDateItem, 'requiredValueLabel');