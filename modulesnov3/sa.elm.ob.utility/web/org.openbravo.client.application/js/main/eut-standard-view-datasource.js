...
/*   var calendar = $.calendars.instance("ummalqura");
          if(Object.prototype.toString.call(data.prop) == '[object Date]'){
        	  ...
        	  if(pDate.getFullYear() >= 1250 && pDate.getFullYear() <=1500 ){
        		  var newJSDate = calendar.toJSDate(pDate.getFullYear(), parseInt(pDate.getMonth())+1, pDate.getDate());
        		  data.prop = newJSDate;
        		  correctedData.prop = newJSDate;
        	  }
          }*/
        }
      }
      ...
      for (prop in data) {
        ...
        correctedData[prop] = data[prop];
        var calendar = $.calendars.instance("ummalqura");
        //console.log("Type: "+Object.prototype.toString.call(data[prop]));
        if (Object.prototype.toString.call(data[prop]) == '[object Date]') {
          var pDate = data[prop];
          if (pDate.getFullYear() >= 1250 && pDate.getFullYear() <= 1500) {
            var isValid = calendar.isValid(pDate.getFullYear(), parseInt(pDate.getMonth()) + 1, pDate.getDate());
            if (isValid) {
              var newJSDate = calendar.toJSDate(pDate.getFullYear(), parseInt(pDate.getMonth()) + 1, pDate.getDate());
              newJSDate.setHours(pDate.getHours());
              newJSDate.setMinutes(pDate.getMinutes());
              newJSDate.setSeconds(pDate.getSeconds());
              correctedData[prop] = newJSDate;
            } else {
              form.setFieldErrors(prop, OB.I18N.getLabel('Eut_invalidHijriDate'), true);
              form.view.messageBar.setMessage(isc.OBMessageBar.TYPE_ERROR, null, OB.I18N.getLabel('Eut_invalidHijriDate'));
