  ...
  initWidget: function () {
    ...
    this.noteListGrid = isc.OBGrid.create({
      ...
      formatCellValue: function (value, record, rowNum, colNum) {
        ...
        /*
         * http://182.18.161.127/mantis/view.php?id=3948
         * 
         * Adding following changes fixes notes creation period issues. 
         * */
        
        var calendar = $.calendars.instance("ummalqura");
        var hijCreationDate = calendar.newDate(record.creationDate.getFullYear(), record.creationDate.getMonth() + 1, record.creationDate.getDate());
        var newJSDate = calendar.toJSDate(hijCreationDate.year(), hijCreationDate.month(), hijCreationDate.day());
        
        newJSDate.setHours(record.creationDate.getHours());
        newJSDate.setMinutes(record.creationDate.getMinutes());
        newJSDate.setSeconds(record.creationDate.getSeconds());
        newJSDate.setMilliseconds(record.creationDate.getMilliseconds());
        
        value = value + ' <span class="OBNoteListGridAuthor">' + OB.Utilities.getTimePassedInterval(record.recordTime - newJSDate.getTime()) + ' ' + OB.I18N.getLabel('OBUIAPP_by') + ' ' + record['createdBy' + OB.Constants.FIELDSEPARATOR + OB.Constants.IDENTIFIER].asHTML() + '</span>';
