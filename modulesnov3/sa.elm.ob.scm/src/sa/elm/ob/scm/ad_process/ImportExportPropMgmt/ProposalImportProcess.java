...
public class ProposalImportProcess extends HttpSecureAppServlet {

  /**
   * Servlet implementation class to import Proposal Lines
   */
  ...
  private static final String includeIn = "../web/sa.elm.ob.scm/jsp/ImportProposal/ImportProposal.jsp";
  ...
  protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    if (inpAction.equals("")) {
      ...
      String inpProposalId = vars.getStringParameter("inpRecordId");
      request.setAttribute("inpProposalId", inpProposalId);
    } else if (inpAction.equals("uploadCsv") || inpAction.equals("validateCsv")) {
      String inpProposalId = request.getParameter("inpProposalId");
      JSONObject isValidCSV = null;
      try {
        // parses the request's content to extract file data

        ProposalImportDAO dao = new ProposalImportDAO(con);
        ...
        if (isMultipart) {
          ...
          if (inpAction.equals("validateCsv")) {
            JSONObject resultJSON = dao.processValidateCsvFile(attachmentFile, vars, inpProposalId,
                false);
            if (resultJSON != null) {
            ...
            if (resultJSON != null) {
              if ("1".equals(resultJSON.getString("status"))) {
                isValidCSV = dao.processUploadedCsvFile(inpProposalId, vars);
