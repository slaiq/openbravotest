...
<HEAD>
<TITLE><%= Resource.getProperty("scm.Itemcard.report",lang) %></TITLE>
      ...
      <FORM id="form" method="post" action="" name="frmMain">
        ...
        <INPUT type="hidden" name="inpWarehouseid" id="inpWarehouseid" value=""></INPUT>
         <INPUT type="hidden" name="inpProductid" id="inpProductid" value=""></INPUT>
                  <INPUT type="hidden" name="inpLocatorid" id="inpLocatorid" value=""></INPUT>
                              ...
                              <div>
                                 <span class="dojoTabcurrentfirst">
                                    <div>
                                       ...
                                       <%= Resource.getProperty("scm.Itemcard.report",lang).replace("'", "\\\'") %>
                                       </a></span>
                                    </div>
                                 </span>
                              </div>
                                    ...
                                    <TD>
                                       ...
                                       <TABLE  align="center" width="100%">
                                          <TR>
                                            ...
                                            <tr>
                                             <td class="TitleCell"><span class="LabelText" id="inpProductIdLabel"> <b> <%=Resource.getProperty("scm.Itemcard.product", lang).replace("'", "\\\'")%>
                                                </b>
                                                </span>
                                             </td>
                                          ...
                                          <tr>
                                             <td class="TitleCell"><span class="LabelText" id="inpWarehouseIdLabel"> <b> <%=Resource.getProperty("scm.Itemcard.warehouse", lang).replace("'", "\\\'")%>
                                                </b>
                                                </span>
                                             </td>
                                             <td class="TextBox_ContentCell">
                                                <select id="inpWarehouseId" class="Combo Combo_TwoCells_width" onchange="getLocator(this.value)">
                                                 <option value="0"><%= Resource.getProperty("scm.--select--",lang).replace("'", "\\\'") %></option>
                                         ...
                                         <%--  <tr>
                                             <td class="TitleCell"><span class="LabelText" id="inpLocatorLabel"> <b> <%=Resource.getProperty("scm.Itemcard.storagebin", lang).replace("'", "\\\'")%>
                                                </b>
                                                </span>
                                             </td>
                                             <td class="TextBox_ContentCell">
                                                <select id="inpLocatorId" class="ComboKey Combo_TwoCells_width">
...
<%  
  if(warehouselist.size() > 0)
  {
    for(CustodyCardReportVO vo:warehouselist)
    { %>
       var warehouse ="<%=vo.getBeneficiaryId()%>";
       getLocator(warehouse);
    <%  break;
    }
  }
%>
function getLocator(warehouse) {
    // load Job No
      $("#inpLocatorId").val("0"); 
        $("#inpLocatorId").next().val($("#inpLocatorId :selected").text());
        
    var url = "<%=request.getContextPath()%>/sa.elm.ob.scm.ad_reports.ItemCardReport/ItemCardReport?inpAction=getLocator&inpWarehouseId=" + warehouse;
    $.getJSON(url, function(result) {

    }).done(function(data) {
        $("#inpLocatorId").find("option").remove().end();
        var select = "<%=Resource.getProperty("scm.select", lang)%>";
        $("#inpLocatorId").append("<option value='0' selected>" + "--" + select + "--" + "</option>");
        $.each(data, function(i, item) {
            $("#inpLocatorId").append("<option value='" + item.id + "'>" + item.value + "</option>");
        });
    });
}
function reloadWindow() {
    submitCommandForm('DEFAULT', false, null, 'ItemCardReport', '_self', null, false);
    return false;
}
...
function generateReport() {
   ...
   // document.getElementById("inpLocatorid").value = document.getElementById("inpLocatorId").value;
    ...
    if (document.getElementById("inpProductid").value == 0) {
        OBAlert('<%=Resource.getProperty("scm.Itemcard.select.product",lang).replace("'","\\\'") %>');
        return false;
    }
    openServletNewWindow('PRINT_PDF', true, null, 'ItemCardReport', null, false, '700', '1000', true);
    return false;
}
(function( $ ) {
    $.widget( "ui.combobox", {
...
/*  $( "#inpWarehouseId").select2(selectBoxAjaxPaging({size: "med/popup",
    placeholder: true})); */
        //  $( "#inpWarehouseId" ).combobox();

             $(document).ready(function() {
                 var green="green";
                 $("#inpProductId").select2(selectBoxAjaxPaging({
                     url : function() {
                         return '<%=request.getContextPath()%>/CustodyCardReportAjax?action=getItemProduct'
                     },
                     size : "small"
                 }));
                 ...
                 $("#inpWarehouseId").select2(selectBoxAjaxPaging({
                     url : function() {
                         return '<%=request.getContextPath()%>/CustodyCardReportAjax?action=getwarehouse'
                     },  size : "med/popup"
                 })); 
