...
<HEAD>
 <META http-equiv="Content-Type" content="text/html; charset=utf-8"></META>
    <TITLE><%=Resource.getProperty("scm.import.purchaseorder.title",lang)%></TITLE>
        ...
        if(status!="" && status!=null){
            if(status=="1") {
                if(statusMessage=="CSV Validated Succesfully")
                {
                    ...
                    displayMessage("S", "Success",statusMessage);
                    var myVar = setInterval(closePopUp, 1000);
                    window.parent.parent.OB.MainView.TabSet.getSelectedTab().pane.activeView.dataSource.view.childTabSet.tabs[0].pane.refresh();
...
<FORM name="frmMain" method="post" action="" enctype="multipart/form-data" id="SubContractForm">
  ...
  <INPUT type="hidden" name="inpOpenEnvelopeEventId" id="inpOpenEnvelopeEventId" value="<%= request.getAttribute("inpOpenEnvelopeEventId")%>"></INPUT>
    ...
    <TD>
      <TABLE cellspacing="0" cellpadding="0" class="Popup_ContentPane_NavBar" id="tdToolBar">
        <TR class="Popup_NavBar_bg"><TD></TD>
          ...
          <TD class="Popup_NavBar_Popup_title_cell"><SPAN><%=Resource.getProperty("scm.import.purchaseorder.title",lang)%></SPAN></TD>
          <TD class="Popup_NavBar_separator_cell"></TD>
        </TR>
      </TABLE>
    </TD>
     ...
     <table cellspacing="0" cellpadding="0" class="Popup_ContentPane_InfoBar">
        <tbody><tr>
          ...
          <td class="Popup_InfoBar_text_table">
            <table>
              <tbody><tr>
                <td class="Popup_InfoBar_text" id="processHelp"><%=Resource.getProperty("scm.oeeimport.title",lang)%> </td>
              </tr>
            </tbody></table>
          </td>
        </tr>
...
function validateFile() {
    ...
    document.forms['frmMain'].action= "<%=request.getContextPath() %>/sa.elm.ob.scm.ad_process.openenvelopeevent/OpenEnvelopeEventImport?action=validateCsv&inpOeeId="+document.getElementById('inpOpenEnvelopeEventId').value;
...
function uploadCSV() {
    ...
    document.forms['frmMain'].action= "<%=request.getContextPath() %>/sa.elm.ob.scm.ad_process.openenvelopeevent/OpenEnvelopeEventImport?action=uploadCsv&inpOeeId="+document.getElementById('inpOpenEnvelopeEventId').value;
    document.forms['frmMain'].submit();
    
}

function closePopUp() {
    document.frmMain.action="<%=request.getContextPath()%>/sa.elm.ob.scm.ad_process.openenvelopeevent/OpenEnvelopeEventImport?action=Close&pageType="+"<%= request.getAttribute("pageType") %>";
