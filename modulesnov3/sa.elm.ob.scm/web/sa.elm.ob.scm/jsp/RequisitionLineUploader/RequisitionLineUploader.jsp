...
<HEAD>
 <META http-equiv="Content-Type" content="text/html; charset=utf-8"></META>
    <TITLE><%=Resource.getProperty("scm.requisition.line.upload",lang)%></TITLE>
...
function onloadFunctions() {
    ...
    <SCRIPT language="JavaScript" type="text/javascript">function onLoadDo(){
        this.windowTables = new Array(
              ...
              xx();
              //$('#inpWindowId').val(window.parent.opener.OB.MainView.TabSet.getSelectedTab().ID);
              var jsonObj = <%= request.getAttribute("Result") %>;
              var isValidated = <%= request.getAttribute("isValidated") %>;
              
              var status = (jsonObj!=null ? jsonObj.status : "");
              var statusMessage = (jsonObj!=null ? jsonObj.statusMessage : "");
              if(status!="" && status!=null){
            	  //window.parent.opener.OB.MainView.TabSet.getTab($('#inpWindowId').val()).pane.activeView.dataSource.view.refreshChildViews();
            	  if(isValidated){
            		  displayMessage("S", "Success",statusMessage); 
            	  } else if("1"==status){
                	  //displayMessage("S", "Success",statusMessage); 
                	  var tabId = window.parent.parent.OB.MainView.TabSet.getSelectedTab().ID
                      window.parent.parent.OB.MainView.TabSet.getTab(tabId).pane.activeView.dataSource.view.messageBar.setMessage('success', statusMessage);
...
<FORM name="frmMain" method="post" action="" enctype="multipart/form-data" id="SubContractForm">
  ...
  <INPUT type="hidden" name="inpRequistionId" id="inpRequistionId" value="${requestScope.inpRequistionId}"></INPUT>
  <INPUT type="hidden" name="inpWindowId" id="inpWindowId" value=""></INPUT>
    ...
    <TD>
      <TABLE cellspacing="0" cellpadding="0" class="Popup_ContentPane_NavBar" id="tdToolBar">
        <TR class="Popup_NavBar_bg"><TD></TD>
          ...
          <TD class="Popup_NavBar_Popup_title_cell"><SPAN><%=Resource.getProperty("scm.requisition.line.upload",lang)%></SPAN></TD>
          <TD class="Popup_NavBar_separator_cell"></TD>
        </TR>
      </TABLE>
    </TD>
     ...
     <table cellspacing="0" cellpadding="0" class="Popup_ContentPane_InfoBar">
        <tbody><tr>
          ...
          <td class="Popup_InfoBar_text_table">
            <table>
              <tbody><tr>
                <td class="Popup_InfoBar_text" id="processHelp"><%=Resource.getProperty("scm.requisition.line.upload",lang)%> </td>
              </tr>
            </tbody></table>
          </td>
        </tr>
...
function validateFileExtension() {
    ...
    if (!(/\.(xlsx)$/i).test(file)) {
          OBAlert('<%= Resource.getProperty("scm.upload.valid.xlsx.file", lang) %>');
        $(file).val('');
        return false;
    }
...
function validateFile() {	
    ...
    document.forms['frmMain'].action= "<%=request.getContextPath() %>/sa.elm.ob.scm.ad_process.RequisitionLineUploader.header/RequisitionLineUploader?action=validateCsv&inpRequistionId="+document.getElementById("inpRequistionId").value;
...
function uploadCSV() {
    ...
    document.forms['frmMain'].action= "<%=request.getContextPath() %>/sa.elm.ob.scm.ad_process.RequisitionLineUploader.header/RequisitionLineUploader?action=uploadCsv&inpRequistionId="+document.getElementById("inpRequistionId").value;    
    document.forms['frmMain'].submit();
    
}
function closePopUp() {
    document.frmMain.action="<%=request.getContextPath()%>/sa.elm.ob.scm.ad_process.RequisitionLineUploader.header/RequisitionLineUploader?action=Close&pageType="+"<%= request.getAttribute("pageType") %>";
