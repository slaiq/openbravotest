                              ...
                              <div>
                                 <span class="dojoTabcurrentfirst">
                                    <div>
                                       ...
                                       <%= Resource.getProperty("scm.custodycard.report",lang).replace("'", "\\\'") %>
                                       </a></span>
                                    </div>
                                 </span>
                              </div>
...
var isWarehouseRole = '<%=request.getAttribute("isWarehouseRole")%>';
var employeeId = <%=request.getAttribute("LoggedInEmployee") %>;

$(document).ready(function (){
	var beneficiaryObject = <%=request.getAttribute("BeneficiaryObj") %>;
	if(isWarehouseRole == 'N'){
		
		$("#inpBeneficiaryType").attr("disabled", true); 
		$("#inpBeneficiaryType").val("E"); 
		
		var data = [{id:employeeId.id, text:employeeId.text}];
		$("#inpBeneficiaryId").select2({data: data});
		$("#inpBeneficiaryId").prop("disabled", true);
 
	}else {
		$("#inpBeneficiaryType").attr("disabled", false);
		$("#inpBeneficiaryId").prop("disabled", false);
	}
});

function reloadWindow() {
    submitCommandForm('DEFAULT', false, null, 'CustodyCardReport', '_self', null, false);
    return false;
}
...
function getbeneficiary(type){
    document.getElementById("inpBeneficiaryId").value = "0";
        $("#inpBeneficiaryId").select2(selectBoxAjaxPaging({
            url : function() {
                return '<%=request.getContextPath()%>/CustodyCardReportAjax?action=getbeneficiary&isWarehouseRole='+isWarehouseRole+'&inptype='+type
           },
           size : "small"
       }));
 }
