...
<HEAD>
 <META http-equiv="Content-Type" content="text/html; charset=utf-8"></META>
    <TITLE><%=Resource.getProperty("scm.import.poreceipt.title",lang)%></TITLE>
            ...
            if(status=="1") {
                if(statusMessage=="CSV Validated Succesfully")
                {
                   ...
                   // displayMessage("S", "Success",statusMessage);
                    ...
                    window.parent.parent.OB.MainView.TabSet.getTab(tabId).pane.activeView.dataSource.view.refreshChildViews();
                    closePage();
                }
            } else if(status=="0") {
...
<FORM name="frmMain" method="post" action="" enctype="multipart/form-data" id="SubContractForm">
  ...
  <INPUT type="hidden" name="inpFormID" id="inpFormID" value="<%=request.getAttribute("inpFormID")%>"></INPUT>
   <INPUT type="hidden" name="inpInOutId" id="inpInOutId" value="<%= request.getAttribute("inpInOutId")%>"></INPUT>
    ...
    <TD>
      <TABLE cellspacing="0" cellpadding="0" class="Popup_ContentPane_NavBar" id="tdToolBar">
        <TR class="Popup_NavBar_bg"><TD></TD>
          ...
          <TD class="Popup_NavBar_Popup_title_cell"><SPAN><%=Resource.getProperty("scm.import.poreceipt.title",lang)%></SPAN></TD>
          <TD class="Popup_NavBar_separator_cell"></TD>
        </TR>
      </TABLE>
    </TD>
     ...
     <table cellspacing="0" cellpadding="0" class="Popup_ContentPane_InfoBar">
        <tbody><tr>
          ...
          <td class="Popup_InfoBar_text_table">
            <table>
              <tbody><tr>
                <td class="Popup_InfoBar_text" id="processHelp"><%=Resource.getProperty("scm.import.poreceipt.title",lang)%> </td>
              </tr>
            </tbody></table>
          </td>
        </tr>
...
function validateFile() {
    ...
    document.forms['frmMain'].action= "<%=request.getContextPath() %>/sa.elm.ob.scm.ad_process.importporeceipt.header/ImportPOReceipt?action=validateCsv&inpInOutId="+document.getElementById('inpInOutId').value;
...
function uploadCSV() {
    ...
    document.forms['frmMain'].action= "<%=request.getContextPath() %>/sa.elm.ob.scm.ad_process.importporeceipt.header/ImportPOReceipt?action=uploadCsv&inpInOutId="+document.getElementById('inpInOutId').value;
    document.forms['frmMain'].submit();
    
}

function closePopUp() {
    document.frmMain.action="<%=request.getContextPath()%>/sa.elm.ob.scm.ad_process.importporeceipt.header/ImportPOReceipt?action=Close&pageType="+"<%= request.getAttribute("pageType") %>";
