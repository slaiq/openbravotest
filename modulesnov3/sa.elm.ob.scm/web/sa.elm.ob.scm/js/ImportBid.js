...
(function() {
  ...
  function openUploadDialog(tabId, recordId) {
    ...
    OB.Layout.ClassicOBCompatibility.Popup.open('escm_bid_import', 900, 650, OB.Utilities.applicationUrl("sa.elm.ob.scm.ad_process.BidManagement.ImportBid/ImportBidLines") + "?Command=" + "DEFAULT" + "&inpRecordId=" + recordId + "&inpTabId=" + tabId + "&inpWindowID=" + windowId + "&pageType=WAD", '', null, false, false, true);
  }

  // register the button for purchase order tab
  // the first parameter is a unique identification so that one button can not be registered multiple times.
  OB.ToolbarRegistry.registerButton('escm_bid_import', isc.OBToolbarIconButton, {
    action: function() {
      var view = this.view,
        grid = view.viewGrid,
        recordId;
      if (view.tabId === '31960EC365D746A180594FFB7B403ABB') {
    ...
    updateState: function() {
      ...
      if (view.tabId === '31960EC365D746A180594FFB7B403ABB') {
        ...
        if (!selectedBidRecord) {
          disableButton(true);
        } else if (selectedBidRecord.bidappstatus == "ESCM_AP" || selectedBidRecord.bidappstatus == "ESCM_IP") {
          disableButton(true);
        } 
        else {
            disableButton(false);
  ...
  OB.ToolbarRegistry.registerButton('escm_bid_import', isc.OBToolbarIconButton, {
    action: function() {
      var view = this.view,
        ...
        tabId = view['tabId'],
        recordId;
      if (view.tabId === 'D54F30C8AD574A2A84999F327EF0E3A4') {
    ...
    buttonType: 'escm_bid_import',
    prompt: OB.I18N.getLabel('escm_bid_import'),
    updateState: function() {
      var view = this.view,
        grid = view.viewGrid,
        me = this,
