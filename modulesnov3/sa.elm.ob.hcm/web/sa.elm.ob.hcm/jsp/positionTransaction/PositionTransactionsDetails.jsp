...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    String lang = ((String)session.getAttribute("#AD_LANGUAGE"));
     ...
     List<PositionTransactionsDetailsVO> gradeList = (ArrayList<PositionTransactionsDetailsVO>) request.getAttribute("inpGradeIdName");
     List<PositionTransactionsDetailsVO> orgList = (ArrayList<PositionTransactionsDetailsVO>) request.getAttribute("inpOrgName");
     List<PositionTransactionsDetailsVO> jobList = (ArrayList<PositionTransactionsDetailsVO>) request.getAttribute("inpJobNo");
  ...
  <BODY onresize="onResizeDo();" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" id="paramBody">
      <FORM id="form" method="post" action="" name="frmMain">
         ...
         <INPUT type="hidden" name="inpAction" id="inpAction" value=""></INPUT>
          ...
          <INPUT type="hidden" name="inpjobId" id="inpjobId" value=""></INPUT> 
                              ...
                              <div>
                                 <span class="dojoTabcurrentfirst">
                                    <div>
                                       ...
                                       <%= Resource.getProperty("hcm.positionTransactionDetail.report",lang).replace("'", "\\\'") %>
                                       </a></span>
                                    </div>
                                 </span>
                              </div>
                                       ...
                                       <TABLE  align="center" width="100%">
                                         ...
                                         <tr>
                                             ...
                                             <td class="TextBox_ContentCell">
                                                <select name="inpGradeName" class="Combo Combo_TwoCells_width required" id="inpGradeName" onchange="getjobNo(this);">
                                                 <option value = "0"><%= Resource.getProperty("hcm.--select--",lang).replace("'", "\\\'") %></option>
                                                
                                                 <%   if(gradeList.size() > 0){
                                                      for(PositionTransactionsDetailsVO vo:gradeList){ %>
                                              ...
                                              <tr>
                                             <td class="TitleCell"><span class="LabelText" id="inpJob"> <b> <%=Resource.getProperty("hcm.job", lang).replace("'", "\\\'")%>
                                                </b>
                                                </span>
                                             </td>
                                             <td class="TextBox_ContentCell">
                                                <select name="inpJobno" class="Combo Combo_TwoCells_width required" id="inpJobno" >
                                                 <option value = "0"><%= Resource.getProperty("hcm.--select--",lang).replace("'", "\\\'") %></option>
                                                
                                                 <%   if(jobList.size() > 0){
                                                      for(PositionTransactionsDetailsVO vo:jobList){ %>
                                                       <option value='<%= vo.getPositionId()%>' ><span><%= vo.getJobNo()%></span></option>
                                                   <%}}%>
                                                </select>
                                             </td>
                                              </tr>
                                           <tr>
                                             ...
                                             <td class="TextBox_ContentCell">
                                                <select name="inpDeptName" class="Combo Combo_TwoCells_width" id="inpDeptName">
                                                 <option value = "0"><%= Resource.getProperty("hcm.--select--",lang).replace("'", "\\\'") %></option>
                                                 <%   if(orgList.size() > 0){
                                                      for(PositionTransactionsDetailsVO vo:orgList){ %>
                                                   ...
                                                   <%  
                                                                    if(request.getAttribute("inpFromDate")!= null ) {%>
                                                                     ...
                                                                     <%}
                                                                      ...
                                                                      <input type="text" id="inpFromDate" class="dojoValidateValid TextBox_btn_OneCell_width required " onchange="document.getElementById('messageBoxID').style.display = 'none';onChangeField(); "  value="" maxlength="10" name="inpFromDate" ></input>
                                                                      <%} %> 
                                             </td>
                                             </tr>
                                             <tr>
                                             <td class="TitleCell"><span class="LabelText" id="inpEndDate"> <b> <%=Resource.getProperty("hcm.ToDate", lang).replace("'", "\\\'")%>
                                              ...
                                              <td class="TextBox_ContentCell">
                                                        ...
                                                        <input type="text" id="inpToDate" class="dojoValidateValid TextBox_btn_OneCell_width" onchange="document.getElementById('messageBoxID').style.display = 'none';onChangeField(); "  value="" maxlength="10" name="inpToDate" ></input>                                                                      
                                               </td>
                                             </tr>
                                          <TR  align = "center">
                                             <td></td>
...
function reloadWindow() {
    submitCommandForm('DEFAULT', false, null, 'PositionTransactionsDetails', '_self', null, false);
    return false;
}
...
function getjobNo() {
    $.post('<%=request.getContextPath()%>/sa.elm.ob.hcm.ad_reports.positionTransaction/PositionTransactionsDetails', {
        inpAction : 'GetJobNo',
        inpGradeName : document.getElementById("inpGradeName").value
    }, function(data) {
         // Getting JobNo
        var JobGroup = data.getElementsByTagName("JobGroup");
       var inpJobNo = document.getElementById("inpJobno");
       inpJobno.options.length = 1;
        for ( var i = 0; i < JobGroup.length; i++) {
              if(i==0)
               {
               var val = JobGroup[i].getElementsByTagName("ID")[0].childNodes[0].nodeValue;

               
               }   
            $(inpJobNo).append($('<option>', {
               
               value : JobGroup[i].getElementsByTagName("ID")[0].childNodes[0].nodeValue,
               html : JobGroup[i].getElementsByTagName("Name")[0].childNodes[0].nodeValue
           })); 
       }
           $('#inpJobno option[value=' + val + ']').attr("selected", "selected");
    });
...
function generateReport() { 
     ...
     document.getElementById("inpjobId").value = document.getElementById("inpJobno").value;
     ...
     if(document.getElementById("inpgradeId").value == "0")
     {
     OBAlert('<%= Resource.getProperty("hcm.gradeId.empty",lang).replace("'", "\\\'") %>');
     return false;
     }
     if(document.getElementById("inpjobId").value == "0")
      {
         OBAlert('<%= Resource.getProperty("hcm.jobId.empty",lang).replace("'", "\\\'") %>');
      return false;
       }
      ...
      openServletNewWindow('PRINT_PDF', true, null, 'PositionTransactionsDetails', null, false, '700', '1000', true);
    return false;
}
