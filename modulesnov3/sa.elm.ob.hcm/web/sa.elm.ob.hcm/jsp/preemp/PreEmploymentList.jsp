...
<BODY onresize="onResizeDo();" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" id="paramBody">
    ...
    <INPUT type="hidden" name="inpPreEmplymentId" id="inpPreEmplymentId" value=""></INPUT>
                            ...
                            <TR><TD>
                                <TABLE class="dojoTabPaneWrapper" border="0" cellpadding="0" cellspacing="0" width="100%" summary="">
                                    <TR><TD>
                                        ...
                                        <td>
                                            <div id="TabqualInfo" class="tabSelected" onmouseover="this.className='tabSelectedHOVER';" onmouseout="this.className='tabSelected';" onclick="reloadTab('EMPQUAL');"><span class="LabelText"><%= Resource.getProperty("hcm.empqualinfo",lang)%></span>
                                            </div>
                                        </td>
                                        <td>
                                        ...
                                        <!-- tab for prevEmployment  -->
                                        <td>
                                            <img  id="ImgPrevEmployment" class="imageSelected"   alt="" src="../web/images/DoubleArrow.png">
                                            </img>
                                        </td>
                                        <td>
                                            <div id="TabPrevEmployment" class="tabCurrent"><span class="LabelText"><%= Resource.getProperty("hcm.emppreemp",lang)%></span>
                                            </div>
                                   ...
                                   <td>
                                     <div id="TabEmpPerPayMethod" class="tabSelected"  onmouseover="this.className='tabSelectedHOVER';" onmouseout="this.className='tabSelected';" onclick="reloadTab('PERPAYMETHOD');"><span class="LabelText"><%= Resource.getProperty("hcm.EmpPerPayMethod",lang)%></span>
                                        ...
                                        <div style="margin-bottom: 5px;"></div>
                                         ...
                                         <div style="width:100%;" id="FormDetails" align="center">
                                                ...
                                                <TABLE style="width:100%; margin-top: 10px;">
                                                    <TR>
                                                        <TD class="TextBox_ContentCell" align="right" style="padding-left: 50px;width: 20px;">
                                        ...
                                        <div id="jqgrid" style="width:100%; display:none;">
                                            <div align="center"><table id="PrevEmploymentList" class="scroll" cellpadding="0" cellspacing="0" width="100%"></table><div id="pager" class="scroll" style="text-align: center;"></div></div>
                                        </div>
                                                           <div style="width:100%;">
...
var currentWindow='EMP',currentTab = 'PREEMP';
var searchFlag = 0, onSearch = 0, onDelete = 0, gridW, gridH;
var formId = "748014FEDECF44D3BA89EDAB65573FE7";
var prevEmploymentGrid = jQuery("#PrevEmploymentList"), lastActiveType = 'Y';
var lang ='<%=lang%>',
direction='ltr';
    if(lang=="ar_SA"){
        direction='rtl';
 ...
 jQuery(function() {
    prevEmploymentGrid.jqGrid({
        direction :direction,
        url : '<%=request.getContextPath()%>/PreEmploymentAjax?action=GetPreEmploymentList',
        colNames : [ 'employeeId','<%= Resource.getProperty("hcm.preemp.empname", lang) %>',
                     '<%= Resource.getProperty("hcm.startdate", lang) %>',
                     '<%= Resource.getProperty("hcm.enddate", lang) %>',
                     '<%= Resource.getProperty("hcm.preemp.position", lang) %>',
                     '<%= Resource.getProperty("hcm.preemp.dep", lang) %>',
                     '<%= Resource.getProperty("hcm.preemp.empcat", lang) %>'],
        colModel : [
                {
                    name : 'employeeId',
                    index : 'employeeId',
                       ...
                       dataInit : function(e) {
                           ...
                           $(e).calendarsPicker({
                               ...
                               onClose : function(dateText, inst) {
                                   if (dateText != "")
                                       prevEmploymentGrid[0].triggerToolbar();
                               }
                           });
                            e.style.width = "25%";
                           setTimeout(
        ...
        loadComplete : function() {
            $("#jqgrid").show();
            ChangeJQGridAllRowColor(prevEmploymentGrid);
        },
        onSelectRow : function(id) {
            ChangeJQGridSelectRowColor(document.getElementById("inpPreEmplymentId").value, id);
            var rowData = prevEmploymentGrid.getRowData(id);
            document.getElementById("inpPreEmplymentId").value= id;
            enableButton("true");
        ...
        beforeRefresh : function() {
            ...
            if (onSearch == 1) {
                prevEmploymentGrid[0].clearToolbar();
                prevEmploymentGrid[0].toggleToolbar();
            }
            onSearch = 0;
            searchFlag = 0;
            reSizeGrid();
        ...
        beforeRequest : function() {
             prevEmploymentGrid.setPostDataItem("inpPreEmplymentId", document.getElementById("inpPreEmplymentId").value);
             
             prevEmploymentGrid.setPostDataItem("inpEmployeeId", document.getElementById("inpEmployeeId").value);
             if ("" + prevEmploymentGrid.getPostDataItem("_search") == "true") {
                    if ("" + prevEmploymentGrid.getPostDataItem("startdate") != "") {
                        var date = "" + prevEmploymentGrid.getPostDataItem("startdate");
                        var validformat = /^\d{1,2}\-\d{1,2}\-\d{4}$/
                        if (!validformat.test(date))
                            prevEmploymentGrid.setPostDataItem("startdate", "");
                        //prevEmploymentGrid.setPostDataItem("startdate_s", ">=");
                    }
                    if ("" + prevEmploymentGrid.getPostDataItem("enddate") != "") {
                        var date = "" + prevEmploymentGrid.getPostDataItem("enddate");
                        var validformat = /^\d{1,2}\-\d{1,2}\-\d{4}$/
                        if (!validformat.test(date))
                            prevEmploymentGrid.setPostDataItem("enddate", "");
                        ///prevEmploymentGrid.setPostDataItem("enddate_s","<=");
                    }
                    prevEmploymentGrid.setPostDataItem("startdate_s", document.getElementById("gs_startdate_s").value);
                    prevEmploymentGrid.setPostDataItem("enddate_s", document.getElementById("gs_enddate_s").value);
             }
             prevEmploymentGrid.setPostDataItem("_search", 'true');
        }
    });
    prevEmploymentGrid.jqGrid('navGrid', '#pager', {
        edit : false,
        add : false,
        del : false,
        search : false,
        ...
        beforeRefresh : function() {
             enableButton("false");
            document.getElementById("gs_startdate_s").value = '=';
            document.getElementById("gs_enddate_s").value = '=';
             if (onSearch == 1) {
                 prevEmploymentGrid[0].clearToolbar();
                 prevEmploymentGrid[0].toggleToolbar();
             }
    ...
    prevEmploymentGrid.jqGrid('filterToolbar', {
        searchOnEnter : false
    });
    prevEmploymentGrid[0].triggerToolbar();
    changeJQGridDisplay("PrevEmploymentList", "inpPreEmplymentId");
}); 
...
function reSizeGrid() {
    ...
    if (gridH < 200)
        gridH = 200;
    prevEmploymentGrid.setGridWidth(gridW, true);
    prevEmploymentGrid.setGridHeight(gridH, true);
}
function onClickEditView() {
    if (document.getElementById('buttonEdition').className == 'Main_ToolBar_Button' && document.getElementById("inpPreEmplymentId").value != "") {
        document.getElementById("inpAction").value = "EditView";
        submitCommandForm('DEFAULT', true, null, 'PreviousEmployment', '_self', null, true);
        return false;
    }
}
function onClickNew() {
...
function onClickDelete() {
    if (document.getElementById('buttonDelete').className == 'Main_ToolBar_Button' && document.getElementById("inpPreEmplymentId").value != "") {
        OBAsk('<%= Resource.getProperty("hcm.confirmdelete", lang) %>', function(result) {
            if (result) {

                xmlhttp = new getXMLObject();
                if (xmlhttp) {
                    var urlPath = "&inpPreEmplymentId=" + document.getElementById("inpPreEmplymentId").value;

                    xmlhttp.open("GET", '<%=request.getContextPath()%>/PreEmploymentAjax?action=DeletePrevEmployment' + urlPath, true);
                    xmlhttp.onreadystatechange = function() {
                        if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                            var resource = xmlhttp.responseXML.getElementsByTagName("DeletePrevEmployment");
                            ...
                            if (response == 'false') {
                                ...
                                document.getElementById("inpPreEmplymentId").value = "";
                                prevEmploymentGrid.trigger("reloadGrid");
                                enableButton("false");
                            }
                        }
                    };
                ...
                document.getElementById(document.getElementById("inpPreEmplymentId").value).focus();
        });
    }
}
