...
<%@page import="sa.elm.ob.hcm.ad_reports.ContractRenewalReport.ContractRenewalVO"%>
...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    List<ContractRenewalVO> linemanagerList =  (List<ContractRenewalVO> )request.getAttribute("inpLineManagerList");
...
<HEAD>
<TITLE><%= Resource.getProperty("hcm.contractrenewal.title",lang) %></TITLE>
                              ...
                              <div>
                                 <span class="dojoTabcurrentfirst">
                                    <div>
                                       ...
                                       <%= Resource.getProperty("hcm.contractrenewal.title",lang).replace("'", "\\\'") %>
                                       </a></span>
                                    </div>
                                 </span>
                              </div>
                                       ...
                                       <TABLE  align="center" width="100%">
                                          ...
                                          <tr>
                                             ...
                                             <td class="TextBox_ContentCell">
                                                <select id="inpEmpNo" class="ComboKey Combo_TwoCells_width" onchange="getDepartmentList(this.value);getLineManagerList(this.value)">
                                          ...
                                          <tr>
                                           ...
                                           <tr>
                                             <td class="TitleCell"><span class="LabelText" id="inpLineManagerLabel"> <b> <%=Resource.getProperty("hcm.Linemanager.title", lang).replace("'", "\\\'")%>
                                                </b>
                                                </span>
                                             </td>
                                             <td class="TextBox_ContentCell">
                                                 ...
                                                 <option value="0"><%= Resource.getProperty("hcm.--select--",lang).replace("'", "\\\'") %></option>
                                                  <% 
                                                      if (linemanagerList!=null && linemanagerList.size() > 0) 
                                                          for(ContractRenewalVO sup : linemanagerList) { %>    
...
function reloadWindow() {
    submitCommandForm('DEFAULT', false, null, 'ContractRenewal', '_self', null, false);
    return false;
}
...
function generateReport() {
    ...
    document.getElementById("inpStartdateH").value = document.getElementById("inpEffectiveDate").value;
    ...
    var  selecteddate =  document.getElementById("inpEffectiveDate").value;
    selecteddate= calendar.newDate(parseFloat(selecteddate.substr(6,4)), parseFloat(selecteddate.substr(3, 2)),parseFloat(selecteddate.substr(0, 2))).add(+1,'m')
    document.getElementById("inpEndDateH").value = selecteddate.formatDate('dd-mm-yyyy');
    ...
    if(valid==1){
        openServletNewWindow('PRINT_PDF', true, null, 'ContractRenewal', null, false, '700', '1000', true);
        return false;
    }
}
...
document.getElementById("select2-inpLineManager-container").style.backgroundColor="#F5F7F1"; 
...
function getDepartmentList(empId){  
    document.getElementById("inpDepartment").value = "00";
         $("#inpDepartment").select2(selectBoxAjaxPaging({
            url : function() {
                return '<%=request.getContextPath()%>/sa.elm.ob.hcm.ad_reports.ContractRenewalReport/ContractRenewal?inpAction=getDepartmentList&empId='+empId
            },
            size : "small"
        }));
        document.getElementById("select2-inpDepartment-container").style.backgroundColor="#F5F7F1";             
...
function getLineManagerList(empId){
	document.getElementById("inpLineManager").value = "00";
	$("#inpLineManager").select2(selectBoxAjaxPaging({
        url : function() {
            return '<%=request.getContextPath()%>/sa.elm.ob.hcm.ad_reports.ContractRenewalReport/ContractRenewal?inpAction=getLineManagerList&empId='+empId
        },
        size : "small"
    }));
    document.getElementById("select2-inpLineManager-container").style.backgroundColor="#F5F7F1"; 
