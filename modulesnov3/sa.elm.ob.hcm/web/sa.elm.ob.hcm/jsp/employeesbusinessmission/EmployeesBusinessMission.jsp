...
<%@ page language="java" contentType="text/html; charset=UTF-8"
    ...
    JSONObject missCatg = (JSONObject) request.getAttribute("MissionCatg");
    JSONArray empArray=empList.getJSONArray("data");
    JSONArray catgArray=missCatg.getJSONArray("data");
    
    /* List<PositionTransactionsDetailsVO> deptList = (ArrayList<PositionTransactionsDetailsVO>) request.getAttribute("DepartmentList"); */
...
<HEAD>
<TITLE><%= Resource.getProperty("hcm.empbusinessmission.title",lang) %></TITLE>
...
function onResizeDo() {
   ...
   <BODY onresize="onResizeDo();" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" id="paramBody">
      <FORM id="form" method="post" action="" name="frmMain">
         ...
         <INPUT type="hidden" name="inpMissCatgId" id="inpMissCatgId" value=""></INPUT>         
                              ...
                              <div>
                                 <span class="dojoTabcurrentfirst">
                                    <div>
                                       ...
                                       <%= Resource.getProperty("hcm.empbusinessmission.title",lang).replace("'", "\\\'") %>
                                       </a></span>
                                    </div>
                                 </span>
                              </div>
                                       ...
                                       <TABLE  align="center" width="100%">
                                          ...
                                          <tr>
                                             ...
                                             <td class="TextBox_ContentCell">
                                                ...
                                                <%--  <%   if(deptList.size() > 0){
                                                      for(PositionTransactionsDetailsVO vo:deptList){ %>                                                      
                                            ...
                                            <tr>
                                             <td class="TitleCell"><span class="LabelText" id="inpMissCatgLabel"> <b> <%=Resource.getProperty("hcm.empbusinessmission.missioncatg", lang).replace("'", "\\\'")%>
                                                </b>
                                                </span>
                                             </td>
                                             <td class="TextBox_ContentCell">
                                                <select id="inpMissCatg" class="ComboKey Combo_TwoCells_width">
                                                 <option value="0"><%= Resource.getProperty("hcm.--select--",lang).replace("'", "\\\'") %></option>
                                                  <%   if(catgArray.length() > 0){
                                                        for (int i = 0, size = catgArray.length(); i < size; i++){
                                                           JSONObject objectInArray = catgArray.getJSONObject(i);%>
                                                    <option value='<%= objectInArray.get("id")%>' ><span><%= objectInArray.get("catgName") %></span></option>
                                                   <%}}%>
                                                </select>
                                             </td>
                                          </tr>
                                           ...
                                           <tr>
                                             ...
                                             <% if(request.getAttribute("inpDate")!= null ) {%>
                                                    <input type="text" id="inpStartDate" class="dojoValidateValid TextBox_btn_OneCell_width " onchange="document.getElementById('messageBoxID').style.display = 'none';onChangeField();"  value="<%= request.getAttribute("inpDate") %>" maxlength="10" name="inpStartDate" ></input> 
                                                <%}
                                                 else{ %>
                                                 <input type="text" id="inpStartDate" class="dojoValidateValid TextBox_btn_OneCell_width  " onchange="document.getElementById('messageBoxID').style.display = 'none';onChangeField(); "  value="" maxlength="10" name="inpStartDate" ></input>
                                                 <%} %> 
                                             </td>
                                          </tr>
...
function reloadWindow() {
    submitCommandForm('DEFAULT', false, null, 'EmployeesBusinessMission', '_self', null, false);
    return false;
}
...
function generateReport() {
    ...
    document.getElementById("inpMissCatgId").value = document.getElementById("inpMissCatg").value;  
    ...
    if (document.getElementById("inpEndDate").value == "") {
        OBAlert('<%=Resource.getProperty("hcm.empbusinessmission.enddatecantbeempty",lang).replace("'","\\\'") %>');
        return false;
    }
    //Checking Date format validation
    var inpEndDate=$("#inpEndDate").val();
    if(checkDateformat(inpEndDate)){
        var inpStartDate=$("#inpStartDate").val();
        if(inpStartDate!=""){
            if(checkDateformat(inpStartDate)){
                inpStartDate = new Date(inpStartDate.substring(3,5)+"/"+inpStartDate.substring(0,2)+"/"+inpStartDate.substring(6,10));
                inpEndDate = new Date(inpEndDate.substring(3,5)+"/"+inpEndDate.substring(0,2)+"/"+inpEndDate.substring(6,10));
                if(inpEndDate < inpStartDate)
                {
                    OBAlert('<%= Resource.getProperty("hcm.enddate<startdate",lang).replace("'", "\\\'") %>');
                    return false;           
                }
            } else
                valid=0;
    ...
    if(valid==1){
        openServletNewWindow('PRINT_PDF', true, null, 'EmployeesBusinessMission', null, false, '700', '1000', true);
        return false;
    }
}
...
$( "#inpDepartment").select2(selectBoxAjaxPaging({size: "small",
    placeholder: true}));
$( "#inpMissCatg").select2(selectBoxAjaxPaging({size: "small",
    placeholder: true}));
...
document.getElementById("select2-inpMissCatg-container").style.backgroundColor="#F5F7F1";
...
function getDepartmentList(empId){
    ...
    setTimeout(function () {
        $("#inpDepartment").select2(selectBoxAjaxPaging({
            url : function() {
                return '<%=request.getContextPath()%>/sa.elm.ob.hcm.ad_reports.employeesbusinessmission/EmployeesBusinessMission?inpAction=getDepartmentList&empId='+empId 
           },
           size : "small"
       }));           
