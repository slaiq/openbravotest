...
<%
    ...
    String  strdependentslist = "";

    String strschemalist ="";
...
<BODY onresize="onResizeDo();" leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" id="paramBody">
    <FORM id="form" method="post" action="" name="frmMain">
        ...
        <INPUT type="hidden" name="inpInsuSchema" id="inpInsuSchema" value="<%= request.getAttribute("inpInsuSchema") %>"></INPUT> 
       ...
       <INPUT type="hidden" name="strcategory" id="strcategory" value=""></INPUT>
       <INPUT type="hidden" name="strdependents" id="strdependents" value=""></INPUT>
       <INPUT type="hidden" name="strschema" id="strschema" value=""></INPUT>
                                                     ...
                                                     <tr id="TabList">
                                                        <td>
                                                            ...
                                                            <td><img  id="ImgqualInfo" class="imageSelected"   alt="" src="../web/images/DoubleArrow.png"></img></td>
                                                             ...
                                                             <td><div id="TabDependent" class="tabSelected"  onmouseout="this.className='tabSelected';" onmouseover="this.className='tabSelectedHOVER';" onclick="reloadTab('Dependent');"><span class="LabelText"><%= Resource.getProperty("hcm.dependent",lang)%></span></div></td>
                                       ...
                                       <div style="width:100%;" id="FormDetails" align="center">
                                        ...
                                        <div id="LoadingContent" style="display:none;">
                                         ...
                                         <div id="jqgrid" style="width:100%; display:none;">
                                            <div align="center">
                                                <table id="MedicalInsuranceList" class="scroll" cellpadding="0" cellspacing="0" width="100%">
                                                </table>
...
direction='ltr';
  ...
  var dependent=  '<%= Resource.getProperty("hcm.dependent",lang)%>';
  var insuanceComName='<%= Resource.getProperty("hcm.insurcompanyname",lang)%>';
  var schema=  '<%= Resource.getProperty("hcm.insuschema",lang)%>';
  var membershipNo='<%= Resource.getProperty("hcm.membershipno",lang)%>';
  var startdate='<%= Resource.getProperty("hcm.startdate",lang)%>';
  var enddate=    '<%= Resource.getProperty("hcm.enddate",lang)%>';
  var insuranceId='<%= Resource.getProperty("hcm.medicalinsuranceId",lang)%>';
  var category='<%= Resource.getProperty("hcm.insuranceCategory",lang)%>';
   var employee= '<%= Resource.getProperty("hcm.employee",lang)%>';
 /* List<EHCMDeflookupsTypeLn> insuranceSchemaList =  (ArrayList<EHCMDeflookupsTypeLn>)request.getAttribute("inpInsuSchema");
 List<EhcmDependents> dependentList = (ArrayList<EhcmDependents>)request.getAttribute("inpDependent");
 List<MedicalInsuranceVO> MedicalInsuVOList = (ArrayList<MedicalInsuranceVO>)request.getAttribute("inpInsuCategory"); */
 
<% List<MedicalInsuranceVO> category = (ArrayList<MedicalInsuranceVO>)request.getAttribute("inpInsuCategory"); 

strcategorylist += '0'+ ":" +   Resource.getProperty("hcm.select",lang) + ";";
  for (MedicalInsuranceVO vo : category){
   strcategorylist += vo.getSearchkey() + ":" +  vo.getName() + ";";
 } 
%>

 if(category!=null && category!=""){
     document.getElementById("strcategory").value="<%=strcategorylist.substring(0,strcategorylist.lastIndexOf(";"))%>"; 
 }
 
<% strdependentslist += "0:" +   Resource.getProperty("hcm.select",lang) + ";";
  JSONObject dependentList = (JSONObject) request.getAttribute("inpDependentId");
  JSONArray dependentArray = new JSONArray();
  if(dependentList != null && dependentList.has("data") && dependentList.length()>0){
    dependentArray= dependentList.getJSONArray("data");
  }
    if(dependentArray.length() > 0){
     for (int i = 0, size = dependentArray.length(); i < size; i++){
        JSONObject objectInArray = dependentArray.getJSONObject(i);
         strdependentslist += objectInArray.get("id")+":" +objectInArray.get("recordIdentifier")+ ";";

}}
if(dependentList!=null && dependentList.length()>0){%>
    document.getElementById("strdependents").value="<%=strdependentslist.substring(0,strdependentslist.lastIndexOf(";"))%>"; 
<%}%>
<% strschemalist += "0:" +   Resource.getProperty("hcm.select",lang) + ";";
JSONObject schemaList = (JSONObject) request.getAttribute("inpInsuSchemaId");
JSONArray schemaArray = new JSONArray();
if(schemaList != null && schemaList.has("data") && schemaList.length()>0){
  schemaArray= schemaList.getJSONArray("data");
}
  if(schemaArray.length() > 0){
   for (int i = 0, size = schemaArray.length(); i < size; i++){
      JSONObject objectInArray = schemaArray.getJSONObject(i);
      strschemalist += objectInArray.get("id")+":" +objectInArray.get("recordIdentifier")+ ";";

}}
if(strschemalist!=null && strschemalist.length()>0){%>
  document.getElementById("strschema").value="<%=strschemalist.substring(0,strschemalist.lastIndexOf(";"))%>"; 
<%}%>
   
 <%-- <% List<MedicalInsuranceVO> dependent = (ArrayList<MedicalInsuranceVO>)request.getAttribute("inpDependent"); 

 strdependentslist += '0'+ ":" +   Resource.getProperty("hcm.select",lang) + ";";
   for (MedicalInsuranceVO vo : dependent){
     strdependentslist += vo.getDependentId() + ":" +  vo.getName() + ";";
  } 
...
<%--  <% List<EHCMDeflookupsTypeLn> schema = (ArrayList<EHCMDeflookupsTypeLn>)request.getAttribute("inpInsuSchema"); 

  strschemalist += '0'+ ":" +   Resource.getProperty("hcm.select",lang) + ";";
    for (EHCMDeflookupsTypeLn vo : schema){
     strschemalist += vo.getId() + ":" +  vo.getName() + ";";
   } 
...
<script type="text/javascript" src="../web/sa.elm.ob.hcm/js/employeemedicalinsuranceList.js?t="<%= new Date() %>"></script>
