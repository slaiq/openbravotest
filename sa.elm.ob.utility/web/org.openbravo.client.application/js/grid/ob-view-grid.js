    ...
    transformData: function (newData, dsResponse) {
      ...
      if (dsResponse && dsResponse.context && (dsResponse.context.operationType === 'fetch' || dsResponse.context.operationType === 'update' || dsResponse.context.operationType === 'add')) {
        if (this.grid) {
          ...
          if (dsResponse.context.operationType === 'fetch' || dsResponse.context.operationType === 'update' || dsResponse.context.operationType === 'add') {
            for (var i = 0; i < newData.length; i++) {

              for (prop in newData[i]) {

                if (prop == 'updated') console.log("Updated Date: " + newData[i][prop]);

                if (Object.prototype.toString.call(newData[i][prop]) == '[object Date]' && prop !== 'updated') {
                  var pDate = newData[i][prop];
                  var newJSDate;
                  if (!(pDate.getFullYear() >= 1250 && pDate.getFullYear() <= 1500)) {
                    newJSDate = calendar.fromJSDate(pDate);
                    var hijDate = new Date();
                    hijDate.setYear(newJSDate.year());
                    hijDate.setMonth(parseInt(newJSDate.month()) - 1);
                    hijDate.setDate(newJSDate.day());

                    hijDate.setHours(pDate.getHours());
                    hijDate.setMinutes(pDate.getMinutes());
                    hijDate.setSeconds(pDate.getSeconds());
                    hijDate.setMilliseconds(pDate.getMilliseconds());

                    newData[i][prop] = hijDate;
                  }
                }
              }
            }
