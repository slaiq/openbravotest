...
OB.ViewFormProperties = {
    ...
    processColumnValue: function(columnName, columnValue, gridEditInformation, mode) {
        ...
        if (columnValue.value && (columnValue.value === 'null' || columnValue.value === '')) {
            ...
            if (isDate) {
                if (isc.unrealdates.indexOf(columnValue.value) > -1) {
                    this.setItemValue(field.name, columnValue.value);
                    if (field.textField) {
    ...
    saveRow: function(parameters) {
        ...
        callback = function(resp, data, req) {
            ...
            if (status === isc.RPCResponse.STATUS_SUCCESS) {
                ...
                if (form.view.isShowingForm) {
                    form.view.viewForm.refresh();
                } else {

                    form.view.viewGrid.refreshGrid();
                }
                keepSelection = true;
    ...
    validateForm: function() {
        var form = this;
        var i, item, length = this.getFields().length,
            value, itemName, dateValidate;
        for (i = 0; i < length; i++) {
            item = this.getItems()[i].name;
            if (this.getFields()[i].type === '_id_15') {
                value = this.getItems()[i].getValue();
                if (value) {
                    if (new Date(value).getFullYear() > 1500 || new Date(value).getFullYear() < 1277) {
                        form.setFieldErrors(item, OB.I18N.getLabel('EUT_YearsInterval'), true);
                        dateValidate = 'N';
                    }
    ...
    allRequiredFieldsSet: function() {
        ...
        for (i = 0; i < length; i++) {
            ...
            var strVal = value + "";
            if ((form.isSaveClicked || form.isNew) && this.isRequired(item) && item.isVisible()) {
