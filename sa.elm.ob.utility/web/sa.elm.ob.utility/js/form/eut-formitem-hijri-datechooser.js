...
// extends standard DateChooser implementation to be used in OBHijriDateItem
/*isc.ClassFactory.defineClass('OBHijriDateChooser', isc.DateChooser);
...
if (isc.OBHijriDateChooser) { // To force SC to load OBDateChooser instead of DateChooser
  isc.DateChooser.getSharedDateChooser = function (properties) {
    return isc.OBHijriDateChooser.create(properties);
  };
}*/
...
isc.ClassFactory.defineClass("OBHijriDateChooser", "VLayout");
...
isc.OBHijriDateChooser.addProperties({
    ...
    dateGridDefaults: {
        ...
        dateClick : function (year, month, date) {
        	...
        	d.setMonth(month);
        	d.setDate(date);
        	var hd = convertGregorianToHijri(d, 'D','D','+0');
            this.creator.dateClick(hd.split("/")[2], hd.split("/")[0], hd.split("/")[1]);*/
...
//!>Deferred
isc.OBHijriDateChooser.addMethods({
    ...
    initWidget : function () {
        ...
        if (this.showNavigationLayout != false) {
            ...
            if (this.showMonthChooser != false) {
                this.addAutoChild("monthChooserButton", {
                    baseStyle:(this.baseNavButtonStyle || this.baseButtonStyle),           
                    title: getShortHijriDayName(this.chosenDate)
                },
                this.navButtonConstructor);
            }
        ...
        if (this.chosenDate) {
            ...
            var convDate = this.getValidYear(this.chosenDate, calendar);
            
            this.hijYear = convDate.year();
            this.hijMonth = convDate.month();
            this.hijDate = convDate;
        }
    ...
    setData : function (data) {
        ...
        var convDate = this.getValidYear(data, calendar);
        ...
        this.dateGrid.setStartDate(convDate);
    },
    
    getValidYear : function (_jsDate, calendar) {
    	var isValid = calendar.isValid(_jsDate.getFullYear(), parseInt(_jsDate.getMonth())+1, _jsDate.getDate()) ;
        var chosenDate ;
		if(!isValid) {
			chosenDate = calendar.newDate().fromJSDate(new Date());
		}
		else {
			chosenDate = calendar.newDate(_jsDate.getFullYear(), parseInt(_jsDate.getMonth())+1, _jsDate.getDate()) ;
		}
    	return chosenDate;
    },
    ...
    getDayNames : function () {
        if (isc.OBHijriDateChooser._dayNames == null) {
            ...
            isc.OBHijriDateChooser._dayNames = [Date.getShortDayNames(1),Date.getShortDayNames(2),Date.getShortDayNames(3)];
        }
        return isc.OBHijriDateChooser._dayNames[this.dayNameLength-1];
    },
...
/*		var calendar = $.calendars.instance("ummalqura");
    	...
    	var isValid = calendar.isValid(this.year, this.month, this.chosenDate.getDate());
		var date;
		if(!isValid) {
			date = calendar.newDate().fromJSDate(new Date());
			this.year = date.year();
			this.month = date.month();
		}
		else {
			date = calendar.newDate(this.year, this.month, this.chosenDate.getDate());
		}
		
		this.updateHeader();
		this.monthChooserButton.setTitle(getShortHijriDayName(date));
		this.yearChooserButton.setTitle("" + this.year);
        if (this.fiscalYearChooserButton) {
            this.fiscalYearChooserButton.setTitle("" + date.year());
        }
        ...
        this.monthChooserButton.setTitle(getShortUmmalQuraMonth(this.hijMonth));
        this.yearChooserButton.setTitle("" + this.hijYear);
        if (this.fiscalYearChooserButton) {
        ...
        var calendar = $.calendars.instance("ummalqura");
     	...
     	var isValid = calendar.isValid(this.hijYear, this.hijMonth, this.hijDate.day());
		var chosenDate;
		if(!isValid) {
			//chosenDate = calendar.newDate().fromJSDate(new Date());
			if( this.hijMonth === 13){
				this.hijMonth =1;
				this.hijYear=this.hijYear+1;
			}
			var date = calendar.newDate(this.hijYear, this.hijMonth, 1);
			chosenDate = calendar.newDate(this.hijYear, this.hijMonth, calendar.daysInMonth(date)); 
		}
		else {
			chosenDate = calendar.newDate(this.hijYear, this.hijMonth, this.hijDate.day());
		}
        
		this.updateGridData(chosenDate);
	},
    ...
    showMonth: function (monthNum) {
        ...
        if (typeof (monthNum) === 'string') {
          this.hijMonth = parseInt(monthNum) + 1;
        } else {
          this.hijMonth = monthNum + 1;
        }
		...
		if (!this.monthMenu) {
			...
			for (var i = 0; i < 12; i++) {
				...
				monthItems[monthItems.length-1].add(
									{	contents:getShortHijriDayName(date),
                                        eventPart: "showMonth",
										eventId: i
									}
					);
	...
	showPrevYear : function () {
		if(this.hijYear > this.startYear) {
			this.hijYear-- ;
			this.year--;
	        this.updateUI();
		}
	...
	showNextYear : function () {
		//var hijYear = this.getValidYear();
        if (this.hijYear < this.endYear) {
        	//(parseInt(this.hijYear))++;
        	this.hijYear ++;
		    this.year++;
            this.updateUI();
        }
	...
	dateClick : function (year, month, day, selectNow) {
        var date = this.chosenDate = Date.createLogicalDate(year, (month-1), day);
		var calendar = $.calendars.instance("ummalqura");
		//var date = this.chosenDate = calendar.newDate(year,month, day);
    ...
    todayClick : function () {
        var date = new Date();
        var calendar = $.calendars.instance("ummalqura");
        var hd = calendar.fromJSDate(date)+"";
        
        this.dateClick(hd.split("-")[0], hd.split("-")[1], hd.split("-")[2], true);
...
isc.OBHijriDateChooser.addClassMethods({
