...
isc.OBAttachmentsLayout.addProperties({
    ...
    callback: function(attachmentsobj) {
        ...
        // Pki integration starts - currently only for purchase invoice header tab
        if (this.attachmentForm.view.tabId === '290' && OB.Properties.EUT_enableDigitalsignature != undefined && OB.Properties.EUT_enableDigitalsignature === 'Y') {

            var recentObj = attachmentsobj.attachments[0],
                age = [];

            for (i in attachmentsobj.attachments) {
                if (attachmentsobj.attachments[i].age < recentObj) {
                    recentObj = attachmentsobj.attachments[i];
                }
            }

            var ipaddress = OB.Properties.EUT_ipaddress,
                tableId = this.getForm().view.standardProperties.inpTableId,
                recordId = this.splitrecordid(tableId, this.attachmentForm.values.id);
            ...
            try {
                var parameters = {
                    ...
                    "filein": {
                        "web": {
                            "url": ipaddress + "/grp/openerp/pki/file-data",
                            "header": {
                                "Authorization": "Basic T3BlbmJyYXZvOnRvbWdycDEyMzU2",
                                "recordId": recordId,
                                "tableId": 318,
                                "fileName": recentObj.name
                            }
                        }
                    },
                    "fileout": {
    ...
    splitrecordid: function(tableid, recordid) {
        var newname = "";
        for (var i = 0; i < recordid.length; i += 3) {
            if (i != 0) {
                newname += "/";
            }
            newname += recordid.substring(i, Math.min(i + 3, recordid.length));
        }
    ...
    fillAttachments: function(attachments) {
        ...
        if (!this.getForm().view.viewForm.readOnly) {
            if (this.tabId === 'A0F3A7D17A834A93B3BD4D2C40E77AFE') {
                if (this.getForm().view.viewForm.view.getContextInfo().inpappstatus !== 'APP') {
                ...
                isc.confirm(OB.I18N.getLabel('OBUIAPP_ConfirmRemoveAll'), function(clickedOK) {
                    if (clickedOK) {
                        OB.RemoteCallManager.call('org.openbravo.client.application.window.AttachmentsAH', {}, d, function(response, data, request) {
                            ...
                            if (!data.deleteFile && !form.view.getContextInfo().inpemEscmPrReturn) {
                                isc.warn(OB.I18N.getLabel('EUT_File_Delete'));
                            }
                        });
                    }
                }, {
        ...
        var downloadActionsfromdms;
        downloadActionsfromdms = function() {
            var d = {
                Command: 'DISPLAY_DATA_DMS',
                inpcFileId: this.attachId
            };
            OB.Utilities.postThroughHiddenForm('./businessUtility/TabAttachments_FS.html', d);
        };

        var sendActionsfromdms;
        sendActionsfromdms = function() {
            var d = {
                Command: 'SEND_DMS',
                inpcFileId: this.attachId,
                tabId: me.tabId,
            ...
            isc.showPrompt(OB.I18N.getLabel('EUT_SENDINGTO_DMS') + isc.Canvas.imgHTML({
        ...
        for (i = 0; i < attachments.length; i++) {
            ...
            var downloadAttachmentfromdms = isc.OBLinkButtonItem.create({
                title: '[ ' + OB.I18N.getLabel('eut_downloadfromdms') + ' ]',
                width: '30px',
                attachmentName: attachment.name,
                attachId: attachment.id,
                action: downloadActionsfromdms
            });

            var sentAttachmentfromdms = isc.OBLinkButtonItem.create({
                title: '[ ' + OB.I18N.getLabel('Eut_Sendtodms') + ' ]',
                width: '30px',
                attachmentName: attachment.name,
                attachId: attachment.id,
                action: sendActionsfromdms
            });
            ...
            if (this.tabId === 'A0F3A7D17A834A93B3BD4D2C40E77AFE' && OB.Properties.Eut_AllowDMSIntegration !== undefined && OB.Properties.Eut_AllowDMSIntegration === 'Y') {
                if (attachment.attachmentPath !== undefined && attachment.attachmentPath !== null) {
                    buttonLayout.addMember(downloadAttachmentfromdms);
                } else {
                    var name = attachment.name;
                    var splitname = name.split('.');
                    if (splitname.length > 0) {
                        if (splitname[splitname.length - 1] === 'pdf') {
                            buttonLayout.addMember(sentAttachmentfromdms);
                        }
                    }
