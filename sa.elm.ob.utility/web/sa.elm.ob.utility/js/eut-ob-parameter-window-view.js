...
isc.OBParameterWindowView.addProperties({
  ...
  initWidget: function () {
    var i, field, items = [],
        ...
        newShowIf, context, updatedExpandSection,processdefId,processmonitorId;
  ...
  doProcess: function (btnValue) {
    ...
    if (this.clientSideValidation) {
      ...
      this.clientSideValidation(this, actionHandlerCall, clientSideValidationFail);
    	...
    	if(this.buttonOwnerView!=undefined && this.buttonOwnerView.viewGrid!=undefined &&  this.buttonOwnerView.viewGrid.getSelectedRecord()!=undefined){
  	  OB.RemoteCallManager.call('sa.elm.ob.utility.ad_actionHandler.ConcurrentExecutionOfProcessDefinition', {
  		 id : this.buttonOwnerView.viewGrid.getSelectedRecord().id,
		  action : "insertProcessMonitor",
		  processId:view.processId,
		  params: allProperties._params
  	  }, {}, function (response, data, request) {
  		  if(data!=null && data!="" && data!=undefined && data.message!=undefined && data.message!=null && data.message!="" && data.message.severity!=undefined){
  			  if(data.message.severity=="success"){
  			      actionHandlerCall();
  			    }
  			  else{
  				view.handleResponse(true, data.message, true, false, data);
  			  }
  		  }
  		  else{
  			 actionHandlerCall();  
