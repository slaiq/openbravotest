...
public class NumberToArabic {
  ...
  /**
   * BHD - not implemented JOD - not implemented
   */
  public static enum Currency {
    AED, SYP, SAR, TND, XAU, JOD, BHD, USD
  }

  private static String[] englishOnes = new String[] { "Zero", "One", "Two", "Three", "Four",
      "Five", "Six", "Seven", "Eight", "Nine", "Ten", "Eleven", "Twelve", "Thirteen", "Fourteen",
      "Fifteen", "Sixteen", "Seventeen", "Eighteen", "Nineteen" };

  private static String[] englishTens = new String[] { "Twenty", "Thirty", "Forty", "Fifty",
      "Sixty", "Seventy", "Eighty", "Ninety" };

  private static String[] englishGroup = new String[] { "Hundred", "Thousand", "Million", "Billion",
      "Trillion", "Quadrillion", "Quintillion", "Sextillian", "Septillion", "Octillion",
  ...
  private static String[] arabicOnes = new String[] { "", "واحد", "اثنان", "ثلاثة", "أربعة", "خمسة",
      "ستة", "سبعة", "ثمانية", "تسعة", "عشرة", "أحد عشر", "اثنا عشر", "ثلاثة عشر", "أربعة عشر",
      "خمسة عشر", "ستة عشر", "سبعة عشر", "ثمانية عشر", "تسعة عشر" };

  private static String[] arabicFeminineOnes = new String[] { "", "إحدى", "اثنتان", "ثلاث", "أربع",
      "خمس", "ست", "سبع", "ثمان", "تسع", "عشر", "إحدى عشرة", "اثنتا عشرة", "ثلاث عشرة", "أربع عشرة",
      "خمس عشرة", "ست عشرة", "سبع عشرة", "ثماني عشرة", "تسع عشرة" };

  private static String[] arabicTens = new String[] { "عشرون", "ثلاثون", "أربعون", "خمسون", "ستون",
      "سبعون", "ثمانون", "تسعون" };

  private static String[] arabicHundreds = new String[] { "", "مائة", "مئتان", "ثلاثمائة",
      "أربعمائة", "خمسمائة", "ستمائة", "سبعمائة", "ثمانمائة", "تسعمائة" };

  private static String[] arabicAppendedTwos = new String[] { "مئتا", "ألفا", "مليونا", "مليارا",
      "تريليونا", "كوادريليونا", "كوينتليونا", "سكستيليونا" };

  private static String[] arabicTwos = new String[] { "مئتان", "ألفان", "مليونان", "ملياران",
      "تريليونان", "كوادريليونان", "كوينتليونان", "سكستيليونان" };

  private static String[] arabicGroup = new String[] { "مائة", "ألف", "مليون", "مليار", "تريليون",
      "كوادريليون", "كوينتليون", "سكستيليون" };

  private static String[] arabicAppendedGroup = new String[] { "", "ألفاً", "مليوناً", "ملياراً",
      "تريليوناً", "كوادريليوناً", "كوينتليوناً", "سكستيليوناً" };

  private static String[] arabicPluralGroups = new String[] { "", "آلاف", "ملايين", "مليارات",
  ...
  public NumberToArabic(BigDecimal number, Currency currency) {
    ...
    numberToArabic(number, currencyInfo, englishPrefixText, englishSuffixText, arabicPrefixText,
        arabicSuffixText);
  }

  public NumberToArabic(BigDecimal number, Currency currency, String englishPrefixText,
  ...
  private static void numberToArabic(BigDecimal number, CurrencyInfo currency,
      String englishPrefixText, String englishSuffixText, String arabicPrefixText,
  ...
  private static String getDecimalValue(String decimalPart) {
    ...
    if (currencyInfo.getPartPrecision() != decimalPart.length()) {
      ...
      int dec = decimalPart.length() <= currencyInfo.getPartPrecision() ? decimalPart.length()
          : currencyInfo.getPartPrecision();
  ...
  public static String convertToEnglish() {
    ...
    formattedNumber += (retVal != "")
        ? (_intergerValue == 1 ? currencyInfo.englishCurrencyName
    ...
    formattedNumber += (decimalString != "")
        ? " " + (_decimalValue == 1 ? currencyInfo.englishCurrencyPartName
  ...
  private static String processArabicGroup(int groupNumber, int groupLevel,
    ...
    if (tens > 0) {
      if (tens < 20) { // if we are processing under 20 numbers
        if (tens == 2 && hundreds == 0 && groupLevel > 0) { // This is special case for number 2
                                                            // when it comes alone in the group
          ...
          if (tens == 1 && groupLevel > 0 && hundreds == 0)
            retVal += " ";
              ...
              && hundreds == 0 && remainingNumber.compareTo(new BigDecimal(0)) == 0)
  ...
  public static String convertToArabic(BigDecimal value, String currencyCode) {
    ...
    List<org.openbravo.model.common.currency.Currency> currencyList = null;
    org.openbravo.model.common.currency.Currency currencyObj = null;
    currency = Currency.valueOf(currencyCode);
    currencyInfo = (new NumberToArabic()).new CurrencyInfo(currency);

    OBQuery<org.openbravo.model.common.currency.Currency> currencyRecord = OBDal.getInstance()
        .createQuery(org.openbravo.model.common.currency.Currency.class,
            "as e where e.iSOCode =:currencyCode");
    currencyRecord.setNamedParameter("currencyCode", currencyCode);
    currencyList = currencyRecord.list();
    if (currencyList.size() > 0) {
      currencyObj = currencyList.get(0);
      currencyInfo.setArabic1199CurrencyName(currencyObj.getDescription());
      currencyInfo.setArabic1CurrencyName(currencyObj.getDescription());
      currencyInfo.setArabic2CurrencyName(currencyObj.getDescription());
      currencyInfo.setArabic310CurrencyName(currencyObj.getDescription());
    }
  ...
  public static String convertToArabic() {
    ...
    while (tempNumber.compareTo(new BigDecimal(0)) > 0) {
      ...
      String groupDescription = processArabicGroup(numberToProcess, group,
          new BigDecimal(Math.floor(tempNumber.doubleValue())));

      if (groupDescription != "") { // here we add the new converted group to the previous
                                    // concatenated text
        if (group > 0) {
          if (retVal != "")
          ...
          if (numberToProcess != 2) {
            if (numberToProcess % 100 != 1) {
              if (numberToProcess >= 3 && numberToProcess <= 10) // for numbers between 3 and 9 we
                                                                 // use plural name
    ...
    formattedNumber += (retVal != "") ? retVal : "";
    if (_intergerValue != 0) { // here we add currency name depending on _intergerValue : 1 ,2 ,
                               // 3--->10 , 11--->99
    ...
    if (_decimalValue != 0) { // here we add currency part name depending on _intergerValue : 1 ,2 ,
                              // 3--->10 , 11--->99
      formattedNumber += " ";
