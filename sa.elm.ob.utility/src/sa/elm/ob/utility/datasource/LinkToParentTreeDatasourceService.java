...
import org.openbravo.service.datasource.TreeDatasourceService;
...
/**
 * This class is used to handle adding, deleting, updating nodes in the tree structure
 * 
...
public class LinkToParentTreeDatasourceService extends TreeDatasourceService {
  ...
  private static String CHILD_IDS = "  ";
  ...
  /**
   * This method is used to handle the node deletion
   * 
   * if node has any children, based on the deletion policy defined in table tree. It will do the
   * following things
   * 
   * 1. Delete Parent And Its Childs - This will delete the childs associated with node deleted 2.
   * 2. Reparent Child Nodes - Restructure the child nodes
   * 
   */
  ...
  protected void deleteNode(JSONObject bobProperties) {
    try {
      ...
      if ("EUT_DPAIC".equals(nodeDeletionPolicy)) {
        deleteChilds(entity, bobParentNode, bobNodeId);
      }

      if ("RCN".equals(nodeDeletionPolicy)) {
  ...
  protected JSONArray fetchNodeChildren(Map<String, String> parameters,
    ...
    if (nResults > nMaxResults) {
      // need to handle this case we have to make TooManyTreeNodesException has public. It is core
      // change
   ...
   * @param parameters
   * @param nodeId
   * @return returns a json object with the definition of a node give its node id @throws
   */
  @Override
   ...
   * @return the number of reparented nodes
   */
  public void deleteChilds(Entity entity, String newParentId, String oldParentId) {
    Table table = OBDal.getInstance().get(Table.class, entity.getTableId());
    Property linkToParentProperty = getLinkToParentProperty(table);
    Property nodeIdProperty = getNodeIdProperty(table);
    Column linkToParentColumn = OBDal.getInstance().get(Column.class,
        linkToParentProperty.getColumnId());
    Column idColumn = OBDal.getInstance().get(Column.class, nodeIdProperty.getColumnId());
    try {
      ConnectionProvider conn = new DalConnectionProvider(false);
      getChildIds(entity, oldParentId, linkToParentProperty);
      if (StringUtils.isNotBlank(CHILD_IDS)) {
        TreeDatasourceServiceData.updateParentInChilds(conn, table.getDBTableName(),
            linkToParentColumn.getDBColumnName(), idColumn.getDBColumnName(),
            CHILD_IDS.replaceFirst(",", ""));
        TreeDatasourceServiceData.deleteChilds(conn, table.getDBTableName(),
            idColumn.getDBColumnName(), CHILD_IDS.replaceFirst(",", ""));
      }
    } catch (ServletException e) {
      logger.error("Error while deleting tree node: ", e);
    }
  ...
  /**
   * This method is used to get all child node hierarchy of given node
   * 
   * @param Entity
   * @param parentId
   * @param linkToParentProperty
  ...
  public void getChildIds(Entity entity, String parentId, Property linkToParentProperty) {

    StringBuilder whereClause = new StringBuilder();
    whereClause.append("as e where e.");
    whereClause.append(linkToParentProperty.getName());
    whereClause.append(".id");
    whereClause.append(" =:parentId");

    OBQuery<BaseOBObject> obQuery = OBDal.getInstance().createQuery(entity.getName(),
        whereClause.toString());

    obQuery.setNamedParameter("parentId", parentId);

    List<BaseOBObject> objectList = obQuery.list();

    if (objectList.size() > 0) {
      for (BaseOBObject object : objectList) {
        CHILD_IDS = CHILD_IDS + ",'" + object.getId() + "'";
        getChildIds(entity, object.getId().toString(), linkToParentProperty);
      }
    }
  }
