...
public class StandardWindowComponent extends BaseTemplateComponent {
  ...
  public OBViewTab getRootTabComponent() {
    ...
    for (Tab tab : getWindow().getADTabList()) {
      ...
      if (processView != null && !"".equals(processView)) {
        /*
         * removed this line for purchase order window loading issue ,becoz its having nearly 7
         * process definition ,two times called getProcessViews()(check line no 144) so added
         * directly processView in processViews list
         */
        // processViews.add(tabComponent.getProcessViews());
        processViews.add(processView);
      }
    }
