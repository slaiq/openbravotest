...
isc.OBFKFilterTextItem.addProperties({
  ...
  getRecordIdsFromIdentifier: function (identifier) {
    var records, recordIds = [],
        i;
    if (this.pickList && this.pickList.data.localData.find(function(data) {
    	   	return data && (data[OB.Constants.IDENTIFIER]).replace(new RegExp('\r?\n','g'), '') === identifier;
    	     })){
    //if(this.pickList && this.pickList.data.find(OB.Constants.IDENTIFIER, identifier)){
     ...
     records = this.pickList.data.localData.findAll(function(data) {
    	      	return data && (data[OB.Constants.IDENTIFIER]).replace(new RegExp('\r?\n','g'), '') === identifier;
    	        });
