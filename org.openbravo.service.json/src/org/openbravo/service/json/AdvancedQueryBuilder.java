...
public class AdvancedQueryBuilder {
  ...
  private String buildFieldClause(List<Property> properties, Property property, String fieldName,
    ...
    if (ignoreCase(properties, operator)) {
      clause = "hqlreplace(upper(" + clause + "))";
    }
  ...
  private Object getTypeSafeValue(String operator, Property property, Object value)
    ...
    if (isLike(operator)) {
      if (operator.equals(OPERATOR_INOTCONTAINS) || operator.equals(OPERATOR_ICONTAINS)
          || operator.equals(OPERATOR_CONTAINSFIELD) || operator.equals(OPERATOR_IEQUALS)) {
    ...
    if (Boolean.class == property.getPrimitiveObjectType()) {
      ...
      try {
        ...
        if (property.isDate()) {
          ...
          if (Integer.parseInt(dateArray[0]) < 1490) {
            if (JsonToDataConverter.unRealDateList.contains((String) value)) {
              date = UtilityDAO.convertToGregorianDate(dateArray[2] + dateArray[1] + dateArray[0]);
            } else {
              date = UtilityDAO.convertToGregorianDate(dateArray[0] + dateArray[1] + dateArray[2]);
            }
          } else {
            date = simpleDateFormat.parse(value.toString());
          }
  ...
  private boolean isLike(String operator) {
    return operator.equals(OPERATOR_ICONTAINS) || operator.equals(OPERATOR_IENDSWITH)
        || operator.equals(OPERATOR_IEQUALS) || operator.equals(OPERATOR_ISTARTSWITH)
        || operator.equals(OPERATOR_CONTAINS) || operator.equals(OPERATOR_ENDSWITH)
        || operator.equals(OPERATOR_STARTSWITH) || operator.equals(OPERATOR_NOTCONTAINS)
        || operator.equals(OPERATOR_INOTCONTAINS) || operator.equals(OPERATOR_NOTENDSWITH)
        || operator.equals(OPERATOR_NOTSTARTSWITH) || operator.equals(OPERATOR_INOTENDSWITH)
        || operator.equals(OPERATOR_INOTSTARTSWITH) || operator.equals(OPERATOR_CONTAINSFIELD)
