...
public class DataToJsonConverter {
  ...
  public Map<String, String> unrealDateMap = new HashMap<String, String>();
  ...
  private void intializeUnrealDateMap() {
    unrealDateMap.clear();
      ...
      for (Property property : bob.getEntity().getProperties()) {
        ...
        if (value != null) {
          if (property.isPrimitive()) {
            // TODO: format!
            if (value.getClass().getName().contains("java.sql.Timestamp")
                && checkDatePresent(value.toString())) {
              jsonObject.put(property.getName(), returnDatePresent(value.toString()));
            } else {
              if (value.getClass().getName().contains("java.sql.Timestamp")) {
                String[] Originaldates = value.toString().split(" ");
                String[] dates = Originaldates[0].split("-");
                if (OBContext.getOBContext().getDates()
                    .get(dates[0] + dates[1] + dates[2]) == null) {
                  value = Timestamp.valueOf(
                      sdf.format(sdf1.parse(Utility.convertToHijriDate(value.toString()))));

                } else {
                  value = Timestamp.valueOf(sdf.format(sdf1
                      .parse(OBContext.getOBContext().getDates().get(dates[0] + dates[1] + dates[2])
                          .split(" ")[0] + " " + Originaldates[1])));
                }
              }
  ...
  private boolean checkDatePresent(String value) {
    String timevalue = value;
    if (timevalue != null) {
      timevalue = value.split(" ").length > 0 ? value.split(" ")[0] : value;
    }
    Set<String> dateSet = unrealDateMap.keySet();
    for (String date : dateSet) {
      if (timevalue.contains(date)) {
        return true;
      }
    }
    return false;
  ...
  private String returnDatePresent(String value) {
    String timevalue = value;
    if (timevalue != null) {
      timevalue = value.split(" ").length > 0 ? value.split(" ")[0] : value;
    }
  ...
  protected Object convertPrimitiveValue(Property property, Object value) {
    ...
    if (Date.class.isAssignableFrom(clz)) {
      ...
      return Base64.encodeBase64String((byte[]) value);
        ...
        .getDomainType() instanceof sa.elm.ob.utility.model.domaintype.SecuredStringDomainType) {
      if (SecuredStringDAO.isAllowedToDisplay(id)) {
        return new String(java.util.Base64.getDecoder().decode(value.toString()));
      } else {
        return "***";
      }
    } else if (property.getDomainType() instanceof StringDomainType) {
      return removeNewline(value.toString());
    }
    return value;
  }
  ...
  public String removeNewline(String value) {
    String str = value;
    try {
      ...
      log.error("Error while finding newline" + e.getMessage());
      return value;
    }
    return str;
  }
